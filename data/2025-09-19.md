...
Motivation: 深度模型在分布偏移下性能下降严重，现有方法依赖多域训练或计算密集型测试时适应，需要更轻量级的解决方案。

Method: 提出Class-Invariant Test-Time Augmentation（CI-TTA），通过弹性与网格变形生成输入图像的类不变变体，并通过置信度引导的过滤方案聚合预测。

Result: 在PACS和Office-Home数据集上，CI-TTA在不同DG算法和骨干网络上均表现出稳定的性能提升。

Conclusion: CI-TTA是一种有效且通用的方法，能够显著提升模型在未见域上的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有分词器通常专注于单一模态的重建或理解，缺乏统一处理多模态的能力。AToken旨在填补这一空白。

Method: 采用纯Transformer架构和4D旋转位置嵌入，结合感知和Gram矩阵损失的无对抗训练目标，逐步扩展训练范围。

Result: 在图像、视频和3D任务中均取得优异性能，如下游生成和理解任务中表现竞争力。

Conclusion: AToken为下一代多模态AI系统提供了统一的视觉分词基础。
...
<details>
  <summary>Details</summary>
Motivation: 解决增量多视图聚类中稳定性与可塑性之间的平衡问题，防止灾难性遗忘。

Method: 提出基于海马体和前额叶皮层记忆机制的三个模块：视图对齐模块、认知遗忘机制和知识巩固模块。

Result: MemEvo在视图数量增长的情况下表现出色，优于现有方法。

Conclusion: MemEvo通过模拟人脑记忆机制，有效解决了增量多视图聚类中的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在融合边缘先验时存在冗余和不稳定优化问题，需要更高效的边缘引导机制。

Method: 结合边缘特征和中间特征激活生成调制图，并集成到轻量级残差设计中，使用复合目标函数训练。

Result: 在标准SISR基准测试中，结构清晰度和感知质量优于SRGAN、ESRGAN等基线方法。

Conclusion: 提出的边缘条件调制方法为感知超分辨率提供了参数高效且稳定的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在迭代去噪过程中对不同噪声水平和模式缺乏高效调度的问题。

Method: 基于分数扩散模型，设计自适应去噪调度和两阶段采样策略，结合特征融合和梯度融合进行迭代去噪。

Result: 在合成和真实扫描数据集上，去噪效果在质量和定量上均优于现有方法，且能更好地保留形状边界和细节。

Conclusion: 提出的自适应迭代方法在点云去噪任务中表现出色，具有高效性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于 SD 地图的定位方法忽略了噪声 GPS 信号，而高精度地图成本高且难以扩展。DiffVL 旨在通过扩散模型联合建模 GPS、SD 地图和视觉信号，解决这一问题。

Method: DiffVL 将视觉定位重新定义为 GPS 去噪任务，通过扩散模型迭代优化噪声 GPS 轨迹，结合 BEV 特征和 SD 地图恢复真实位姿分布。

Result: 在多个数据集上，DiffVL 实现了优于传统 BEV 匹配方法的定位精度，达到亚米级水平。

Conclusion: DiffVL 证明了扩散模型可以通过将噪声 GPS 作为生成先验，实现可扩展的视觉定位，为传统匹配方法提供了新的范式转变。
...
<details>
  <summary>Details</summary>
Motivation: 稀疏视图CT重建因采样不足导致病态逆问题，传统方法难以捕捉复杂结构，扩散模型作为生成先验表现出潜力。

Method: 提出DICE框架，通过交替执行数据一致性代理（近端算子）和先验代理（扩散模型）来平衡生成先验与测量一致性。

Result: 在15、30和60视图的稀疏设置下，DICE显著优于现有基线，重建出高质量CT图像。

Conclusion: DICE有效结合生成先验与测量一致性，提升了稀疏视图CT重建的性能和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法因设备差异和临床设置不同导致域偏移，且目标域缺乏监督或标注成本高。

Method: 使用共享聚合令牌和最大严重性三元组损失对齐跨域类别分布。

Result: 实验表明该方法优于其他域适应方法。

Conclusion: 该方法有效解决了域偏移问题，提升了UC严重程度估计的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 研究城市场景的阅读方式可以为设计和规划提供参考，但目前缺乏针对城市感知任务的标准化评估方法。

Method: 使用100张蒙特利尔街道图像（50张照片和50张合成图像），收集了12名参与者对30个维度的230份标注（物理属性和主观印象），评估了7种VLMs在零样本设置下的表现。

Result: 模型在可见、客观属性上的表现优于主观评价；最佳模型（claude-sonnet）在多标签任务上的宏平均得分为0.31，平均Jaccard重叠为0.48；合成图像略微降低模型表现。

Conclusion: 研究提出了一个可重复、不确定性感知的评估基准，适用于参与式城市分析，并发现人类一致性较高的任务中模型表现更好。
...
<details>
  <summary>Details</summary>
Motivation: 动态点云压缩依赖精确运动估计，但现有方法难以捕捉复杂动态和充分利用时间相关性。

Method: FMT采用时空对齐策略，隐式建模连续时间变化，并设计随机访问参考策略支持双向运动引用和分层编码。

Result: 实验显示FMT在编解码效率和BD-Rate上优于D-DPCC和AdaDPCC，分别降低20%和9.4%。

Conclusion: FMT有效提升了压缩效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决CNN在长距离依赖建模上的不足，同时避免Transformer在高分辨率医学图像中的计算开销，并防止过度关注全局上下文导致的局部结构信息丢失。

Method: 采用双互补机制：1) 特征扫描策略，结合轴向遍历和局部自适应路径；2) 门控模块结合空间-频率分析。

Result: 在MRI和CT数据集上表现优于现有方法。

Conclusion: HybridMamba在3D医学图像分割中表现出色，解决了边界模糊和区域失真问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统跳跃连接存在特征间和特征内的约束，限制了信息传递和多尺度特征整合的效果。

Method: DSC块包含测试时训练（TTT）模块和动态多尺度核（DMSK）模块，分别解决特征间和特征内约束。

Result: 实验证明DSC块在多种U型网络中均有效，具有即插即用的优势。

Conclusion: DSC块显著提升了跨层连接的自适应性，增强了多尺度特征整合能力。
...
Method: 提出LSTC模块并行处理短长期时序特征，并通过JMDA数据增强技术扩展样本多样性。

Result: 在多个数据集上取得SOTA结果，如NTU 60（X-Sub 94.1%，X-View 97.5%）。

Conclusion: LSTC-MDA通过改进时序建模和样本多样性，显著提升了骨架动作识别性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有指令图像编辑方法在复杂任务上表现不佳，且数据集存在噪声和偏见，限制了模型能力。

Method: 利用多模态大语言模型生成视觉自适应编辑指令并生成高质量编辑图像。

Result: 在MultiEdit-Test基准测试中，模型性能显著提升，同时保持标准编辑能力。

Conclusion: MultiEdit为研究多样化和挑战性指令图像编辑提供了宝贵资源。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法因缺乏适应性而难以应用于复杂模型，需要一种更灵活的解释生成方法。

Method: 引入Attention Lattice Adapter (ALA)和Alternating Epoch Architect (AEA)机制，ALA简化层选择，AEA通过交替更新参数解决注意力区域过小问题。

Result: 在CUB-200-2011和ImageNet-S数据集上，方法在多项指标上优于基线，尤其在CUB-200-2011上平均IoU提升53.2点。

Conclusion: 提出的方法有效提升了视觉基础模型的解释生成能力和适应性，具有显著性能优势。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有深度学习方法在遮挡、姿态变化和视角变化时的局限性。

Method: 融合基础模型的低分辨率语义特征和CNN的高分辨率空间特征，支持任意数量的参考图像。

Result: 定量和定性评估显示，使用多参考图像能显著提升上色性能。

Conclusion: DACoN通过结合语义和空间特征，实现了更鲁棒和精细的线稿上色。
...
<details>
  <summary>Details</summary>
Motivation: 单目视频中几何信息不足，现有3D高斯方法难以保留表面细节，需要新的表示方法和信息补充。

Method: 提出Mesh-Guided 2D Gaussian Splatting，将2D高斯基元约束到模板网格上，并利用基础模型补充视觉信息，通过选择性梯度隔离协调优化目标。

Result: 实验显示FMGS-Avatar在几何精度和外观保真度上优于现有方法，且支持新视角和姿态下的一致性渲染。

Conclusion: FMGS-Avatar通过增强表示和协调知识蒸馏，显著提升了单目3D人体化身重建的质量和语义丰富性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法未充分利用MLLM的多模态推理能力，导致图像检索性能不佳。

Method: 设计了列表排序提示和图像评估提示，使MLLM能够进行全局比较和一致推理，并引入查询解构提示进行细粒度分析。

Result: 在五个数据集上的实验表明，CoTRR在三种图像检索任务中达到了最先进的性能。

Conclusion: CoTRR方法通过直接利用MLLM的推理能力，显著提升了图像检索的准确性和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 历史艺术品中气味相关物体的检测面临标注稀疏和类别极度不平衡的问题，合成数据生成有望缓解这些挑战。

Method: 评估多种基于扩散模型的数据增强策略，并将合成数据融入模型训练。

Result: 实验表明，合成数据能显著提升检测性能，尤其在标注稀缺的领域效果显著。

Conclusion: 扩散模型的大规模预训练为标注稀缺领域的物体检测提供了有效解决方案，且在小数据量下仍表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频基准测试存在帧采样偏差问题，不同帧选择策略导致模型性能评估不准确。

Method: 提出帧级准确的基准测试，控制帧采样策略，评估小规模视觉语言模型在视频问答中的表现。

Result: 结果证实了帧采样偏差的存在，并展示了不同帧采样技术下模型的数据和任务特异性行为。

Conclusion: 开源基准测试代码，呼吁未来研究针对不同数据集制定标准化的帧采样策略。
...
Method: 使用高斯混合模型分割点云，平面部分用2D体素边界描述，曲面部分用B样条曲面拟合。

Result: 在多个公开数据集上表现优于现有方法，效率提升3.78倍，精度提高2倍，实时运行速度为36.4 fps。

Conclusion: 该方法在实时性和精度上取得了显著改进，适用于低功耗设备。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有GAN反转数据蒸馏方法依赖预训练模型和类别信息的问题，提高通用性和适用性。

Method: 提取高梯度图像块，基于CLIP特征分类，用扩散模型学习分布并合成蒸馏图像。

Result: 仅用0.68%原始数据训练，性能仅下降0.3 dB，计算时间大幅缩短。

Conclusion: 新方法在数据效率和计算效率上均优于现有技术，适用于资源受限场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖简化提示，忽略了放射学报告中的丰富语义细节，导致文本图像对齐和临床保真度不足。

Method: 结合BiomedVLP CXR BERT、MedEmbed和ClinicalBERT三种预训练医学文本编码器，利用放射学报告和体素间距信息训练3D潜在扩散模型。

Result: 生成的CT体积具有解剖一致性和高质量，在多编码器条件下CLIP分数提升，在VLM3D挑战赛中排名第一。

Conclusion: Report2CT通过完整放射学报告和多编码器文本条件，推动了3D CT合成的临床保真度和高质量数据生成。
...
<details>
  <summary>Details</summary>
Motivation: 传统测地线主动轮廓模型在骨分割中因特征提取不区分而难以处理边缘阻塞、泄漏和骨折问题。

Method: 结合骨科知识，构建了新的边缘检测函数，融合强度和梯度信息，并引入距离信息作为自适应步长以稳定轮廓演化。

Result: 在骨盆和踝关节分割实验中表现出色，解决了上述问题，展现了准确、稳定和一致的性能。

Conclusion: 该算法不仅适用于多种骨解剖结构，还为结合领域知识与深度神经网络提供了思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有学习型CSR方法在变形能量优化和训练一致性方面存在挑战，需要改进。

Method: 设计了MED损失函数作为变形轨迹的正则化项，结合V2C-Flow模型使用。

Result: 在不影响重建精度和拓扑正确性的前提下，显著提升了训练一致性和可重复性。

Conclusion: MED损失函数有效解决了学习型CSR中的变形优化问题，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI方法在骨健康研究中主要依赖视觉数据且缺乏解释性，ProtoMedX旨在解决这一问题，满足医疗应用和欧盟AI法案的要求。

Method: ProtoMedX采用基于原型的架构，结合DEXA扫描和患者记录，设计上具有可解释性。

Result: 在4,160名NHS患者数据集上，ProtoMedX在仅视觉任务中准确率为87.58%，多模态版本为89.8%，均优于现有方法。

Conclusion: ProtoMedX在骨健康分类中表现出色，同时提供可解释的决策分析，适合医疗应用。
...
<details>
  <summary>Details</summary>
Motivation: 城市数字化需求增加，手动更新数据库效率低，需自动化解决方案。

Method: 利用Metric Depth Estimation模型，结合几何原理和相机参数计算坐标。

Result: 通过LiDAR点云验证，模块在不同距离和语义区域表现良好。

Conclusion: MapAnything有效自动化城市物体和事件映射，适用于实际场景。
...
Result: 在五个传统基准和数据集上优于之前的正则化方法。

Conclusion: 该框架为超分辨率模型的泛化问题提供了有效且通用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 验证arXiv:2311.17983中提出的DDS是否能提升Vision Transformers在分割和分类任务中的可解释性鲁棒性，并扩展研究其普适性和计算成本。

Method: 复现FViTs和DDS方法，测试其在基线方法和Attribution Rollout上的效果，并评估计算成本和环境影响。

Result: 结果与原研究基本一致，DDS确实提升了可解释性鲁棒性，但存在微小差异。

Conclusion: DDS是一种有效的提升Vision Transformers可解释性鲁棒性的方法，但需注意其计算成本和环境影响。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像分类方法依赖参数密集型训练，而现有视觉语言模型（VLMs）因单次表示限制难以捕捉视觉内容的互补性。

Method: MARIC通过Outliner Agent分析全局主题并生成提示，三个Aspect Agent提取细粒度描述，Reasoning Agent整合输出进行分类。

Result: 在4个基准数据集上，MARIC显著优于基线方法。
...
Method: 通过从乳腺磁共振成像（MRI）数据中学习潜在空间中的轨迹变化，构建多任务模型，预测治疗成功反应。

Result: 在ISPY-2数据集上，仅使用治疗前数据（T0）的平衡准确率为0.761，使用早期反应数据（T0 + T1）为0.811，使用四个时间点数据（T0 -> T3）为0.861。

Conclusion: 该方法通过潜在轨迹空间中的线性分类器，能够有效预测乳腺癌患者的治疗反应，且随着时间点增加，预测准确率提高。
...
<details>
  <summary>Details</summary>
Motivation: 解决数据驱动的航天器姿态估计方法在实际任务中应用受限的问题，因其决策过程不透明。

Method: 通过训练基于NeRF的图像生成器，利用姿态估计网络的梯度回传来渲染主要3D特征。

Result: 实验表明该方法能恢复相关3D线索，并揭示姿态估计网络监督与其隐式表示的关系。

Conclusion: 该方法为理解姿态估计网络的决策提供了新视角，有助于提升其在实际任务中的可信度。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂视频对象分割中的小目标、遮挡、快速运动和复杂交互问题。

Method: 采用伪标签训练策略和级联决策机制，结合SAM2Long和SeC模型的优势。

Result: 在MOSE测试集上J&F得分为0.8616，比基线提升1.4分。

Conclusion: 方法在长复杂视频分割中表现出强鲁棒性和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 研究CLIP模型在微调后对跨域泛化能力的影响，特别是在高度专业化的生物识别任务中。

Method: 评估三个微调后的CLIP模型（FR、MAD、PAD）在14个通用视觉数据集上的表现，并与原始CLIP模型对比。

Result: 微调后的模型在专业任务上表现优异（如FR任务提升58.52%），但在通用任务上性能显著下降（如ImageNetV2下降至51.63%）。

Conclusion: 模型容量越大，越能缓解过专化问题；任务复杂性和分类头设计影响遗忘程度。
...
<details>
  <summary>Details</summary>
Motivation: 与人类KOL合作成本高且存在物流挑战，需要一种更高效的解决方案。

Method: 通过模块化服务（如服装生成、妆容转移等）实现虚拟KOL图像的动态生成。

Result: 系统能够显著降低品牌内容制作成本并加速营销流程。

Conclusion: GenKOL为营销专业人士提供了一种灵活、可扩展的虚拟KOL生成方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有检测模型仅提供简单的真实性分类或二元判断，缺乏解释性；MLLM方法虽可解释但准确性不足。

Method: 提取MLLMs的潜在知识并通过提示注入训练，提升LLaVA的检测能力。

Result: DF-LLaVA在合成图像检测中超越专家模型，同时保持高可解释性。

Conclusion: DF-LLaVA在准确性和可解释性上均表现优异，为合成图像检测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D增量学习方法在极端数据稀缺下表现不佳，主要由于几何错位和纹理偏差，导致语义模糊和灾难性遗忘。

Method: CMGR框架包含结构感知几何校正模块和纹理增强模块，通过分层对齐3D结构与CLIP的空间先验，并合成判别性纹理。

Result: 实验表明，CMGR显著提升了3D少样本增量学习的性能，增强了几何一致性和对纹理偏差的鲁棒性。

Conclusion: CMGR通过跨模态几何校正和纹理增强，有效解决了3D增量学习中的几何和纹理问题，提升了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 标准欧几里得GNN难以准确表示大脑网络的层次拓扑结构，限制了其在临床中的应用。

Method: 基于双曲几何（Lorentz模型）的框架，采用双曲图注意力层和带符号聚合机制，处理兴奋和抑制连接。

Result: 在两个大规模fMRI数据集上的实验表明，该方法在精神病分类任务中显著优于现有方法。

Conclusion: 该研究开创了fMRI分析的新几何深度学习范式，展示了双曲GNN在计算精神病学中的巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 电商产品类别快速增长导致物品识别困难，传统2D方法因类内变异和视觉相似性表现不佳。

Method: 第一阶段用大视觉模型提取2D特征生成候选排名；第二阶段通过3D特征质量预测和几何感知匹配提升准确性。

Result: RoboEye在Recall@1上比现有技术（RoboLLM）提升7.1%，且仅需RGB图像。

Conclusion: RoboEye通过动态结合2D和3D特征，显著提升识别性能，同时降低部署成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖随机掩码策略，忽略了几何结构和语义连贯性，无法捕捉跨方向的空间关系和语义对象部分。

Method: 采用双流掩码方法：3D空间网格掩码通过坐标排序创建结构化模式，渐进语义掩码通过注意力驱动聚类发现语义部分。

Result: 在ModelNet40、ScanObjectNN和OmniObject3D数据集上表现出色，显著优于基线旋转不变方法。

Conclusion: 提出的方法作为即插即用组件，无需架构更改即可集成到现有框架中，具有广泛兼容性和性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统超声诊断依赖医生经验、主观性强且效率低的问题，以及现有通用视觉语言模型在超声医学任务中知识有限、泛化能力差的问题。

Method: 采用Mixture of Experts架构，训练数据涵盖七个解剖区域，支持超声报告生成、诊断和视觉问答等多任务。

Result: 在超声报告生成任务中，BLEU-1和ROUGE-1分数分别比Qwen2-VL提高了10.15和4.77分。

Conclusion: EchoVLM在提升超声诊断准确性方面具有潜力，为未来临床应用提供了可行的技术方案。
...
<details>
  <summary>Details</summary>
Motivation: 手动3D建模耗时且费力，现有生成方法在视觉质量、多样性和语义一致性方面存在挑战，缺乏高质量数据集。

Method: 利用包含12,328个结构化标注场景的数据集，开发了SpatialGen模型，通过3D布局和参考图像生成多模态输出。

Result: SpatialGen在实验中表现优于现有方法，能够生成视觉质量高且语义一致的3D场景。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像检索方法在零售场景中难以区分高度相似的产品，而现有模型（如CLIP和SigLIP）无法捕捉细微差异，像素级匹配又计算成本高。

Method: PRISM分为三阶段：1) 使用SigLIP初步检索相似产品；2) 用YOLO-E去除背景干扰；3) 通过LightGlue进行像素级匹配。

Result: 在ABV数据集上，PRISM的top-1准确率比现有方法高4.21%，且满足实时处理需求。

Conclusion: PRISM通过混合方法有效解决了零售产品检索中的挑战，兼顾了精度和效率。
...
<details>
  <summary>Details</summary>
Motivation: 电线因其细长轮廓在无人机避障中难以检测，需要创新解决方案。

Method: 利用合成数据训练的时序相关层，处理电线检测和深度估计的联合任务。

Result: 模型在电线检测和深度估计任务中优于现有方法。

Conclusion: 该模型有望提升无人机安全性和精确性，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法常忽略高浑浊环境中的距离依赖性可见性损失。

Method: 改进的合成数据生成流程，包含前向散射项和非均匀介质，并收集真实浑浊数据。

Result: 在浑浊度增加时表现优于参考模型，82.5%的参与者选择率。

Conclusion: 改进的模型在浑浊环境中表现更优，数据和代码已公开。
...
Conclusion: 知识蒸馏在缺失模态场景中表现优越，为方法选择和未来研究提供指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本引导图像编辑方法需要大量手动调整超参数，计算成本高。

Method: 使用强化学习框架，将超参数搜索建模为马尔可夫决策过程，动态调整去噪步骤中的超参数。

Result: 实验表明，该方法显著减少了搜索时间和计算开销。

Conclusion: 该方法推动了扩散模型图像编辑框架的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决合成数据与真实数据之间的域差距问题，探索仅使用合成数据训练目标检测模型的可行性。

Method: 采用YOLOv11模型，结合数据增强、数据集多样化和模型缩放策略，通过合成数据和域随机化进行训练。

Result: 最佳配置的YOLOv11l模型在Kaggle竞赛隐藏测试集上达到0.910的mAP@50分数。

Conclusion: 增加合成数据多样性和精细调优数据增强是缩小域差距的关键，但完全捕捉真实世界变异性仍具挑战。
...
<details>
  <summary>Details</summary>
Motivation: 确定深度学习模型在不同疾病严重程度和病理类型中的表现，以支持肺移植术前规划。

Method: 回顾性研究32例患者CT扫描，使用Unet-R231、TotalSegmentator和MedSAM模型，定量和定性评估性能。

Result: Unet-R231表现最佳，但所有模型在严重病例中性能显著下降。

Conclusion: 需针对严重病理优化模型，Unet-R231和TotalSegmentator是当前最佳选择。
...
<details>
  <summary>Details</summary>
Motivation: 探索多模态视觉信息的灵活预训练与微调流程，以增强语义分割的鲁棒性。

Method: 构建ImageNeXt多模态数据集，设计高效预训练方法，支持任意模态组合。

Result: 在多个多模态语义分割数据集上达到新SOTA。
...
Method: 第一阶段使用伪标签监督微调（SFT）引入外部知识；第二阶段采用GRPO和四种奖励信号促进自洽推理。

Result: 在三个医学QA基准上，MEDFACT-R1比之前最优方法提升了22.5%的事实准确性。
...
<details>
  <summary>Details</summary>
Motivation: 探索感知心理学启发的归纳偏置对深度学习模型的潜在提升。

Method: 提出合成几何错觉数据集，结合ImageNet分类任务，评估三种多源学习策略。

Result: 几何错觉辅助监督提升泛化能力，尤其在复杂轮廓和纹理场景；感知偏置增强CNN和Transformer的结构敏感性。

Conclusion: 感知科学与机器学习的结合为视觉模型设计提供了新方向。
...
TL;DR: 论文提出了一种针对检索增强生成（RAG）系统的新型攻击方法AIP，通过操纵指令提示来影响检索行为，揭示了RAG系统中被忽视的漏洞。
...
Method: 提出AIP攻击，利用遗传算法优化对抗性指令提示，平衡攻击效果与隐蔽性。

Result: AIP攻击成功率高达95.23%，同时保持正常功能。
...
### [56] [RynnVLA-001: Using Human Demonstrations to Improve Robot Manipulation](https://arxiv.org/abs/2509.15212)
...
TL;DR: RynnVLA-001是一种基于大规模视频生成预训练的视觉-语言-动作模型，通过两阶段预训练方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 通过结合视觉帧预测与动作预测，提升视觉-语言-动作模型的初始化效果。

Method: 提出两阶段预训练方法：1) 自我中心视频生成预训练；2) 人类中心轨迹感知建模，并引入ActionVAE压缩动作序列。

Result: 在相同下游机器人数据集上微调后，RynnVLA-001性能优于现有基线。

Conclusion: 两阶段预训练策略为视觉-语言-动作模型提供了更有效的初始化方法。
...
TL;DR: M&N框架通过从2D预训练模型向3D分割模型渐进蒸馏知识，提升半监督3D医学图像分割性能。


<details>
  <summary>Details</summary>
Motivation: 解决半监督3D医学图像分割中标注数据稀缺的问题。

Method: 提出模型无关框架，通过伪掩码协同训练和自适应学习率采样优化训练过程。

Result: 在多个公开数据集上优于13种现有方法，达到SOTA性能。

Conclusion: M&N具有模型无关性，可适应未来更先进的模型架构。
...
### [58] [Out-of-Sight Trajectories: Tracking, Fusion, and Prediction](https://arxiv.org/abs/2509.15219)
*Haichao Zhang,Yi Xu,Yun Fu*

Main category: cs.CV

TL;DR: 论文提出了一种新的任务OST，用于预测视线外物体的无噪声轨迹，通过视觉-定位去噪模块在无监督方式下处理噪声数据，并在多个数据集中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖完整无噪声的观测数据，忽视了视线外物体和传感器噪声的挑战，导致实际场景中的预测不可靠。

Method: 提出视觉-定位去噪模块，利用相机标定建立视觉-定位映射，无监督去噪噪声数据。

Result: 在Vi-Fi和JRDB数据集上取得最优性能，显著超越基线方法。

Conclusion: 该工作是首次将视觉-定位投影用于视线外物体的轨迹去噪，为未来研究奠定了基础。
...
TL;DR: 提出了一种无种族偏见的RA-GAN模型，用于生成同年龄的父母与子女照片，提升亲缘关系验证的准确性。


<details>
  <summary>Details</summary>
Motivation: 亲缘关系验证中，父母与子女的照片通常存在年龄差距，且同年龄照片难以获取。现有的人脸老化模型存在种族偏见，影响照片相似度。

Method: 设计了RA-GAN模型，包含RACEpSp和特征混合器模块，生成无种族偏见的同年龄照片，并用于亲缘关系验证。

Result: RA-GAN在种族准确性上优于SAM-GAN和CUSP-GAN，且在亲缘关系验证中显著提升了准确性，特别是在60+年龄组。

Conclusion: RA-GAN能有效消除种族偏见，提升亲缘关系验证的准确性，尤其在处理不同年龄段的父母与子女照片时表现优异。
...
TL;DR: 利用多模态大语言模型（MLLMs）提出零样本时空视频定位（STVG）框架，通过分解时空高亮（DSTH）和时间增强组装（TAS）策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 探索MLLMs在STVG任务中的零样本能力，发现其动态分配特殊标记（grounding tokens）但存在推理不足的问题。

Method: 提出DSTH策略分解查询为属性和动作子查询，使用LRA模块学习空间和时间提示；TAS策略通过时间增强帧提升时间一致性。

Result: 在多个STVG基准测试中优于现有方法。

Conclusion: MLLMs通过DSTH和TAS策略可有效提升STVG任务的零样本性能。
...
TL;DR: MSDD是一个高质量玉米幼苗航空图像数据集，用于精准农业中的幼苗计数，支持早期作物监测和资源优化。


<details>
  <summary>Details</summary>
Motivation: 传统玉米幼苗计数方法费时且易错，需要高效准确的计算机视觉解决方案。

Method: 使用YOLO系列模型（如YOLOv9和YOLO11）在MSDD数据集上进行检测，涵盖多种生长阶段和环境条件。

Result: 单株检测精度高达0.984，但多株检测因数据不平衡和外观不规则而表现较差。YOLO11推理速度最快（35ms/图）。

Conclusion: MSDD为精准农业中的实时决策和资源优化提供了可靠基础，推动了农业监测自动化。
...
TL;DR: 论文提出了一种自监督训练框架ST-AR，解决了自回归模型在视觉领域应用中的三个关键问题，显著提升了图像理解和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有自回归模型在视觉领域面临局部依赖、语义不一致和空间不变性不足等问题，限制了其图像理解能力。

Method: 引入自监督目标，提出ST-AR训练框架，无需预训练模型即可提升性能。

Result: ST-AR显著提升了LlamaGen-L和LlamaGen-XL的生成质量，FID分别提升42%和49%。

Conclusion: ST-AR框架有效解决了自回归模型在视觉领域的局限性，提升了图像理解和生成能力。
...
TL;DR: SyncSeal是一种专为图像同步设计的鲁棒水印方法，通过嵌入和提取网络增强现有水印方法对几何变换的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有水印方法对几何变换（如裁剪、旋转）的鲁棒性不足，SyncSeal旨在解决这一问题。

Method: 使用嵌入网络和提取网络端到端训练，结合判别器保持图像质量，预测几何变换参数。

Result: 实验验证了SyncSeal在多种几何变换下的有效性，并能提升现有水印方法的鲁棒性。

Conclusion: SyncSeal能有效增强水印方法对几何变换的鲁棒性，具有实际应用价值。
...
TL;DR: 论文提出了一种基于扩散模型的多视图立体（MVS）框架，通过条件扩散过程优化深度图，提高了计算效率和性能。


<details>
  <summary>Details</summary>
Motivation: 传统MVS方法在计算效率和性能上存在局限，扩散模型在生成任务中的成功启发了其在MVS中的应用。

Method: 将深度优化建模为条件扩散过程，设计了条件编码器和轻量级扩散网络（2D U-Net和卷积GRU），并提出了基于置信度的采样策略。

Result: DiffMVS在运行时间和GPU内存上具有竞争力，CasDiffMVS在多个数据集上达到SOTA性能。

Conclusion: 扩散模型在MVS中表现出色，新框架显著提升了效率和性能。
...
TL;DR: ScaleCUA是一个开源计算机使用代理（CUA）的扩展项目，通过大规模数据集和闭环管道提升跨平台操作能力，显著超越基线性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型（VLMs）在计算机使用代理（CUA）中的应用受限于缺乏大规模开源数据和基础模型，ScaleCUA旨在解决这一问题。

Method: ScaleCUA通过结合自动化代理和人类专家的闭环管道，构建了一个跨6个操作系统和3个任务领域的大规模数据集，并基于此训练模型。

Result: ScaleCUA在多个基准测试中表现优异（如WebArena-Lite-v2提升26.6，ScreenSpot-Pro提升10.7），并创下新记录（如MMBench-GUI L1-Hard达到94.4%）。

Conclusion: 数据驱动的扩展对通用计算机使用代理具有显著效果，ScaleCUA的数据、模型和代码将开源以推动未来研究。
...
TL;DR: 提出一种跨模态蒸馏范式，利用视觉基础模型（VFM）生成密集代理标签，解决事件相机数据缺乏密集深度标注的问题。


<details>
  <summary>Details</summary>
Motivation: 事件相机在高速运动和强光变化环境下表现优异，但缺乏密集深度标注的大规模数据集限制了基于学习的单目深度估计。

Method: 通过空间对齐的事件流和RGB帧，利用VFM的鲁棒性生成代理标签，并改进VFM（如Depth Anything v2）或设计新型循环架构。

Result: 在合成和真实数据集上验证，跨模态范式性能接近全监督方法，且VFM模型达到SOTA性能。

Conclusion: 跨模态蒸馏和VFM改进有效解决了事件相机深度估计的标注问题，并提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决开放词汇语义分割中无源域适应的挑战，提升伪标签生成和模型效率。

Method: 采用师生范式，结合词汇对齐策略和LoRA微调，引入Top-K类选择机制降低内存需求。

Result: 在CityScapes数据集上提升6.11 mIoU，在零样本分割基准测试中表现优异。
...
TL;DR: CalibPrompt框架通过提示调优校准医学视觉语言模型的置信度，提升临床决策可靠性。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型的置信度校准未被充分探索，可能导致过度自信的错误，影响临床信任。

Method: 提出CalibPrompt框架，通过优化可学习提示和设计校准目标（如平滑准确率对齐和角度分离损失）来校准模型。

Result: 在四个Med-VLMs和五个医学影像数据集上，CalibPrompt显著改善校准效果，且不影响干净准确率。

Conclusion: CalibPrompt是首个在提示调优中校准Med-VLMs的框架，有效提升置信度可靠性。
...
TL;DR: 提出了一种基于LLM的自动生成数学猜想并证明的框架，通过上下文学习提升证明能力。


<details>
  <summary>Details</summary>
Motivation: 探索LLM在发现新定理方面的潜力，而不仅仅是解决已有问题。

Method: 采用Conjecturing-Proving Loop流程，结合上下文学习生成和证明猜想。

Result: 成功重新发现并验证了未形式化的已发表定理，证明了上下文学习的有效性。

Conclusion: 上下文学习对神经定理证明具有显著效果，框架具有实际应用潜力。
...
TL;DR: 研究了深度神经网络（DNNs）在近似非局部守恒定律解中的效率，重点评估了网络架构选择对解精度的影响。


<details>
  <summary>Details</summary>
Motivation: 探索DNNs在科学计算中的应用，特别是解决具有挑战性的物理系统问题。

Method: 通过系统实验，分析了激活函数、网络深度、宽度和训练方法对收敛特性的影响。

Result: tanh激活函数表现稳定，sin激活函数在某些情况下误差更低但可能产生非物理伪影；DNNs在最优配置下与传统数值方法竞争。

Conclusion: DNNs在科学计算中具有潜力，但标准前馈架构在处理不连续解时存在局限性，需进一步突破。
...
<details>
  <summary>Details</summary>
Motivation: 揭示神经网络在二元监督下学习置信度估计的根本限制，解释其信息理论约束。

Method: 通过数学分析和实验验证，包括负奖励训练、对称损失函数和后校准方法。

Result: 实验显示所有方法均失败，后校准虽部分成功但依赖变换而非学习。

Conclusion: 提出新监督范式（如集成分歧和多智能体学习）以突破限制。
...
TL;DR: 论文提出Q-ROAR方法，解决RoPE位置插值与后训练量化结合时的精度下降问题，通过频带分组和搜索优化恢复性能。


<details>
  <summary>Details</summary>
Motivation: 扩展LLM上下文窗口对长程任务至关重要，但RoPE插值与量化结合会导致精度下降，需系统性解决方案。

Method: 提出Q-ROAR方法，分组RoPE维度为频带，搜索优化每频带尺度，无需微调或架构改动。

Result: Q-ROAR恢复0.7%标准任务精度，降低GovReport困惑度10%以上，保持短上下文性能。

Conclusion: Q-ROAR有效解决插值与量化耦合问题，兼容现有推理框架，提升长上下文性能。
...
TL;DR: Hashing-Baseline是一种无需训练的哈希方法，利用预训练编码器生成紧凑二进制嵌入，适用于图像和音频检索。
...
Method: 结合PCA、随机正交投影和阈值二值化等经典技术，利用预训练编码器的冻结嵌入生成哈希。

Result: 在标准图像检索和新型音频哈希基准测试中表现竞争性。

Conclusion: Hashing-Baseline是一种通用且有效的训练免费哈希方法。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端参与分布与优化目标分布不一致导致偏差和不稳定更新。

Method: 提出FedAVOT，将聚合问题建模为掩码最优传输问题，使用Sinkhorn缩放计算权重。

Result: FedAVOT在非光滑凸FL设置下实现标准收敛速率，实验显示性能显著优于FedAvg。

Conclusion: FedAVOT在异构、公平敏感和低参与率场景下表现优异。
...
TL;DR: 介绍了一种轻量级异常检测算法H-Alpha Anomalyzer，用于处理GONG网络的Hα观测数据，提供可解释性并优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 随着天文观测数据量的激增，确保输入机器学习模型的数据质量至关重要，尤其是GONG网络的Hα观测数据。

Method: 提出了一种非机器学习的轻量级异常检测算法，可基于用户定义标准识别异常，并量化异常可能性。

Result: 算法在2000个观测数据（异常与非异常各半）上表现优于现有方法，并提供可解释性。

Conclusion: H-Alpha Anomalyzer不仅高效且可解释，适合领域专家进行定性评估。
...
TL;DR: 论文提出了一种随机局部协调方法，通过减少通信需求来优化分布式优化问题。


<details>
  <summary>Details</summary>
Motivation: 解决分布式优化中全同步通信效率低下的问题，通过局部协调降低通信成本。

Method: 每个节点独立随机选择一个正则化项，仅与共享该正则化项的节点协调，减少通信量。

Result: 在凸目标下达到$\tilde{\mathcal{O}}(\varepsilon^{-2})$迭代次数，强凸性下达到$\mathcal{O}(\varepsilon^{-1})$或$\mathcal{O}(\log(1/\varepsilon))$收敛。

Conclusion: 该方法有效降低了通信需求，同时保持了收敛性能，适用于实际数据集。
...
TL;DR: BEACON是一种基于大语言模型（LLM）和1D CNN的新型深度学习框架，用于行为恶意软件分类，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统静态分析难以应对现代恶意软件的复杂性和规避技术，行为检测提供了更可靠的解决方案。

Method: 利用LLM从沙箱行为报告中生成上下文嵌入，再通过1D CNN进行多分类。
...
TL;DR: 提出了一种预测业务流程中活动开始和结束时间的技术，支持资源容量规划。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅预测活动序列的单一时间戳，无法满足资源容量规划需求。

Method: 采用扫掠线方法，同步预测所有进行中案例的活动后缀，包括等待和处理时间。

Result: 在真实和合成数据集上验证了多模型方法的准确性优势。

Conclusion: 多模型方法在预测活动时间方面具有显著优势。
...
Result: LiMuon在非凸随机优化中实现了较低的样本复杂度（O(ε⁻³)），并在广义平滑条件下保持高效。

Conclusion: LiMuon在训练DistilGPT2和ViT模型时表现出高效性，验证了其优越性。
...
TL;DR: 提出了一种自适应增强的自监督学习框架（A²SL），通过检索相关观测样本增强目标生态系统的建模，显著提高了数据稀缺和非典型场景下的预测准确性。


<details>
  <summary>Details</summary>
Motivation: 环境知识发现依赖标注数据，但数据收集成本高，现有机器学习方法在数据稀疏或非典型条件下泛化能力差。

Method: 设计了多层级对学习损失训练场景编码器，捕获场景间相似性，并通过检索机制补充目标场景数据；还设计了自适应增强机制处理非典型场景。

Result: 在淡水生态系统中，A²SL显著提高了水温和溶解氧动态的预测准确性，增强了数据稀缺和非典型场景下的鲁棒性。

Conclusion: A²SL框架不仅适用于淡水生态系统，还可广泛应用于其他科学领域。
...
<details>
  <summary>Details</summary>
Motivation: 传统PINN方法在不确定性估计方面存在不足，E-PINN旨在提供更可靠的校准覆盖概率。

Method: 利用证据深度学习的边际分布损失函数，推断PDE未知参数的后验分布。

Result: 在1D Poisson方程和2D Fisher-KPP方程上，E-PINN的校准覆盖概率显著优于贝叶斯PINN和深度集成方法。

Conclusion: E-PINN在理论和实际应用中均表现出色，适用于临床数据分析等领域。
...
TL;DR: SPMD-LRT是一种直接处理张量数据的结构保持边缘分布学习方法，通过低秩分解优化边缘分布，提高了分类精度。
...
Method: SPMD-LRT结合一阶和二阶张量统计量（边缘均值和方差），并利用低秩分解技术（如CP和Tucker分解）参数化权重张量，采用交替优化算法求解。

Result: 在多种数据集上的实验表明，SPMD-LRT的分类精度优于传统SVM、向量化LMDM和先前的张量SVM扩展方法。

Conclusion: SPMD-LRT在处理高维张量数据时表现出高效性和鲁棒性，特别是Tucker分解版本效果最佳。
...
TL;DR: 提出了一种基于稀疏高斯混合模型Q函数（S-GMM-QFs）的结构化在线策略迭代框架，用于强化学习。


<details>
  <summary>Details</summary>
Motivation: 扩展离线训练的GMM-QFs，开发在线方案以利用流数据促进探索。

Method: 通过Hadamard过参数化实现稀疏化，调节模型复杂度，避免过拟合。参数更新采用黎曼流形上的在线梯度下降。

Result: S-GMM-QFs在标准基准测试中性能与密集深度强化学习方法相当，但参数更少，且在低参数情况下仍表现良好。

Conclusion: S-GMM-QFs是一种高效且可解释的强化学习框架，适用于资源受限场景。
...
TL;DR: 论文提出了一种结合能量匹配项的损失函数，用于改进粗粒度机器学习模型在分子动力学模拟中的采样能力，但未显著提升准确性。


<details>
  <summary>Details</summary>
Motivation: 传统力匹配方法在捕捉热力学景观时存在不足，无法准确拟合低能构象态之间的绝对差异。

Method: 在损失函数中引入能量匹配项，并使用CGSchNet模型在Chignolin蛋白上进行评估。

Result: 能量匹配未显著提高准确性，但揭示了模型在自由能表面泛化上的不同趋势。
...
TL;DR: PM-SFL是一种隐私保护的Split Federated Learning框架，通过概率掩码训练和个性化掩码学习解决数据重构攻击和异构性问题。


<details>
  <summary>Details</summary>
Motivation: 传统Split Federated Learning存在隐私风险（如数据重构攻击）和性能下降问题，需要一种既能保护隐私又能保持模型性能的方法。

Method: 提出PM-SFL框架，结合概率掩码训练和个性化掩码学习，并引入分层知识补偿机制以应对系统异构性。

Result: 理论分析验证了隐私保护能力，实验表明PM-SFL在准确性、通信效率和隐私攻击鲁棒性上表现优异。

Conclusion: PM-SFL有效解决了隐私和异构性问题，提升了Split Federated Learning的实用性和安全性。
...
TL;DR: HD3C是一种轻量级分类框架，用于低功耗环境，比传统深度学习模型更节能且精度接近。


<details>
  <summary>Details</summary>
Motivation: 现代疾病筛查需要高效节能的分类方法，尤其是在嵌入式设备普及的家庭和野外医疗场景中。

Method: HD3C通过将数据编码为高维超向量，聚合为多个聚类原型，并在超空间中进行相似性搜索来完成分类。

Result: 在心脏声音分类任务中，HD3C比Bayesian ResNet节能350倍，精度差异小于1%。

Conclusion: HD3C在噪声、有限训练数据和硬件错误下表现优异，适合实际部署。
...
TL;DR: CUFG（Curriculum Unlearning via Forgetting Gradients）是一种新的机器学习遗忘框架，通过梯度校正和渐进式遗忘策略，提升模型遗忘的稳定性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘方法过于注重效率和激进遗忘，导致模型权重不稳定和可靠性下降。

Method: CUFG结合了基于遗忘梯度的梯度校正器和从易到难的渐进式遗忘策略。

Result: CUFG缩小了与黄金标准Retrain方法的差距，提升了遗忘的有效性和可靠性。

Conclusion: CUFG为机器学习遗忘领域提供了前瞻性思路，实验验证了其有效性和潜力。
...
TL;DR: 提出了一种基于离散小波变换的动态小波位置编码（DyWPE），显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有位置编码方法仅依赖序列索引，忽略了信号特性，尤其在时间序列分析中表现不佳。

Method: 利用离散小波变换（DWT）直接从输入时间序列生成位置嵌入。

Result: 在10个时间序列数据集上，DyWPE平均相对改进9.1%，计算效率保持竞争力。

Conclusion: DyWPE是一种信号感知的位置编码框架，显著提升了时间序列分析的性能。
...
TL;DR: DeCoP提出了一种依赖控制预训练框架，通过显式建模动态多尺度依赖关系，解决了时间序列预训练中的分布偏移和多尺度模式问题。


<details>
  <summary>Details</summary>
Motivation: 时间序列中的动态时间依赖性由于分布偏移和多尺度模式而难以建模，现有方法无法有效捕捉短期和长期依赖的复杂交互，导致泛化能力下降。

Method: DeCoP通过实例级块归一化（IPN）缓解分布偏移，并采用分层依赖控制学习（DCL）策略显式建模多尺度依赖关系，结合实例级对比模块（ICM）增强全局泛化能力。

Result: DeCoP在十个数据集上取得了最先进的结果，计算资源更低，例如在ETTh1上MSE提高了3%，仅使用PatchTST 37%的FLOPs。

Conclusion: DeCoP通过显式建模动态多尺度依赖关系，显著提升了时间序列预训练模型的泛化能力和计算效率。
...
TL;DR: 提出一种基于时钟机制的注意力模型，用于连续有序序列的对齐任务，优于标准点积注意力。


<details>
  <summary>Details</summary>
Motivation: 标准点积注意力缺乏对连续性和单调性的显式建模，而这对帧同步任务至关重要。

Method: 通过学习非负时钟源和目标，将注意力建模为时钟相遇概率，推导出高斯式评分规则，支持并行和自回归解码。

Result: 在文本到语音任务中，模型生成更稳定的对齐，对全局时间缩放更鲁棒，且性能优于基线。

Conclusion: 该框架适用于其他连续目标（如视频和时序信号建模），是一种高效且通用的对齐方法。
...
Method: 提出DSCL框架，包含基于奖励的动态采样和基于任务的动态课程学习。

Result: 在BFCLv3基准上提升3.29%性能。
...
TL;DR: 论文提出了一种名为PreGC的图压缩方法，通过最优传输和预训练技术，解决了传统图压缩方法对任务和架构依赖的限制，提升了泛化能力和复用性。


<details>
  <summary>Details</summary>
Motivation: 传统图压缩方法依赖特定任务和架构，限制了其复用性和泛化能力。本文从GNN优化一致性的角度重新审视图压缩目标，提出了一种更通用的优化范式。

Method: 提出了PreGC方法，结合最优传输和混合间隔图扩散增强技术，通过优化表示传输计划保持语义一致性，并设计了可追溯的语义协调器。

Result: 实验证明PreGC在任务无关性和与任意GNN的兼容性方面表现出色。

Conclusion: PreGC通过最优传输和预训练技术，显著提升了图压缩的泛化能力和复用性，适用于多种任务和架构。
...
TL;DR: 利用transcoder从单细胞基础模型（scFM）中提取可解释的决策电路，验证其对应真实生物机制。
...
Result: 提取的电路与真实生物机制一致，证明transcoder的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有锚图多视图聚类方法存在信息冗余和噪声干扰问题，且后处理步骤独立导致效率低下。
...
TL;DR: FlowCast-ODE是一个用于小时级天气预报的深度学习框架，通过连续流建模和ODE求解器解决误差累积问题，性能优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在6小时间隔预测中表现良好，但小时级预测仍面临误差累积和数据同化周期不连续性的挑战。

Method: 提出FlowCast-ODE框架，通过连续流建模和ODE求解器实现时间一致性预测，并引入低秩AdaLN-Zero调制机制减小模型规模。

Result: 实验显示FlowCast-ODE在RMSE和能量守恒方面优于基线，且能有效预测极端天气事件。

Conclusion: FlowCast-ODE解决了小时级天气预报中的误差累积问题，性能优越且模型轻量化。
...
TL;DR: 论文探讨了在固定数据和无限计算资源下如何优化语言模型预训练，提出了改进的调整正则化方法，并发现集成模型能显著降低损失。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决计算资源增长快于可用文本数据的问题，探索在数据受限情况下的预训练优化方法。

Method: 方法包括调整正则化参数（如权重衰减）、参数扩展、集成模型以及知识蒸馏。

Result: 结果显示，优化后的方法在200M tokens下数据效率提高了5.17倍，集成模型显著降低损失，知识蒸馏保留了83%的集成优势。

Conclusion: 结论表明，简单的算法改进可以在计算资源丰富的未来显著提高数据效率，适用于下游任务。
...
TL;DR: 提出了一种结合结构先验的序列药物-靶点相互作用框架，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 药物-靶点相互作用（DTI）的准确识别是计算药理学中的核心挑战，序列方法具有可扩展性。

Method: 整合结构先验到蛋白质表示中，同时保持高通量筛选能力。

Result: 在Human和BioSNAP数据集上达到SOTA，在BindingDB上表现竞争性；虚拟筛选中优于先前方法。

Conclusion: 框架在可扩展和结构感知的DTI预测中具有实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测模型的评估方法缺乏标准化，尤其是在异构交通场景和联合预测模型方面存在不足。

Method: 提出了STEP框架，提供统一的数据集接口、一致的训练和评估条件，并支持多种预测模型。

Result: 实验揭示了现有测试程序的局限性、联合建模的重要性以及当前先进模型对分布偏移和对抗攻击的脆弱性。

Conclusion: STEP旨在从“排行榜”方法转向更深入的模型行为和泛化能力分析，适用于复杂多智能体场景。
...
TL;DR: 论文提出了一种基于精度矩阵的神经网络（PNNs），通过联合学习网络参数和精度矩阵，解决了传统协方差神经网络（VNNs）的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统协方差矩阵通常是密集的，无法编码条件独立性，且通常是任务无关的预计算，可能影响性能。

Method: 通过交替优化方法，联合学习网络参数和精度矩阵，并理论分析了估计精度矩阵与真实矩阵的距离。

Result: 实验证明，联合估计方法在合成和真实数据上优于两步法。

Conclusion: PNNs通过精度矩阵实现了更高效和任务相关的学习。
...
TL;DR: 提出了一种基于扩散模型的多变量预测框架（DST），用于构建情景树，并在能源市场中验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 在不确定系统中（如能源市场和金融），随机预测对高效决策至关重要，需要估计未来情景的完整分布。

Method: DST通过扩散模型递归采样未来轨迹，并通过聚类将其组织成树状结构，确保每个阶段的非预期性。

Result: 在纽约州日前电力市场的能源套利任务中，DST优于传统模型和模型无关的强化学习基线。

Conclusion: DST在随机优化中表现更优，能更好地处理不确定性，生成更高效的决策策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习需要与高保真模拟器交互，成本高；离线强化学习受限于数据集质量。多保真度模拟器提供了新的优化机会。

Method: 通过信息增益最大化的自举方法选择模拟器保真度，结合离线和在线数据优化策略。

Result: 理论分析证明MF-HRL-IGM具有无遗憾性，实验显示其性能优于现有基准。

Conclusion: MF-HRL-IGM在多保真度环境下高效优化策略，为实际应用提供了可行方案。
...
TL;DR: G2LFormer提出了一种新的全局到局部注意力机制，解决了传统图Transformer中局部信息丢失的问题。


<details>
  <summary>Details</summary>
Motivation: 传统图Transformer中全局注意力机制可能稀释局部邻域信息，导致信息丢失。

Method: G2LFormer采用浅层网络捕获全局信息，深层网络学习局部结构信息，并通过跨层信息融合策略保留全局层的有益信息。

Result: G2LFormer在节点级和图级任务中表现优异，同时保持线性复杂度。

Conclusion: G2LFormer通过全局到局部注意力机制有效平衡了全局和局部信息，提升了性能。
...
TL;DR: DPANet的关键组件通过消融实验验证，双域融合和交叉注意力机制是性能提升的核心。
...
Method: 设计了Temporal-Only和Frequency-Only模型，并移除了交叉注意力机制进行对比。

Result: 完整模型表现最佳，双域融合和交叉注意力机制对性能至关重要。

Conclusion: 双域信息和交互融合模块是DPANet的核心创新点。
...
TL;DR: 论文探讨了在存在隐藏节点的情况下，如何通过学习部分观测的低通滤波图信号来恢复真实图拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 研究隐藏节点对图学习的影响，并验证现有方法在部分观测下的鲁棒性。

Method: 通过扩展Dirichlet能量函数的受限等距性质（RIP），分析基于平滑度的图学习方法（如GL-SigRep）。

Result: 理论证明和实验验证表明，部分观测下仍能恢复真实图拓扑。

Conclusion: 现有图学习方法在部分观测下具有隐含鲁棒性，适用于低通滤波信号。
...
TL;DR: 论文提出了一种结合强化学习和遗传算法的新方法，用于系统识别影响美强子衰变测量的关键背景过程，并适用于其他粒子物理测量。


<details>
  <summary>Details</summary>
Motivation: 美强子衰变实验面临大量背景干扰，传统方法依赖物理学家直觉且计算受限，缺乏系统性解决方案。

Method: 利用强化学习（RL）和遗传算法（GA）协同，通过GA高效探索轨迹空间，RL代理结合Transformer架构处理衰变序列。

Result: 新方法能系统识别关键背景，解决了稀疏奖励和大轨迹空间的挑战。

Conclusion: 该方法不仅适用于美强子衰变，还可推广到其他粒子物理测量，为背景分析提供了系统性工具。
...
TL;DR: 提出了一种计算持久图鲁棒Wasserstein重心的一般方法，适用于q>1的运输成本，特别是对异常值鲁棒的q∈(1,2)。


<details>
  <summary>Details</summary>
Motivation: 经典方法仅适用于q=2的Wasserstein距离，限制了其应用范围。

Method: 采用固定点方法计算适用于通用运输成本的重心图。

Result: 在持久图聚类和字典编码中展示了其对异常值的鲁棒性。
...
TL;DR: 提出了一种基于自解释神经网络（SENN）的方法，用于增强强化学习任务的解释性，同时保持预测准确性。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习缺乏透明度，降低了在关键领域中的可信度。

Method: 结合SENN和解释提取方法，针对低维问题生成局部和全局解释。

Result: 在移动网络资源分配问题上验证了SENN的竞争性能和解释性。

Conclusion: SENN在低维任务中能提升AI决策的透明度和信任度，性能与现有方法相当。
...
TL;DR: 论文提出了一种名为DAG的双因果网络框架，用于时间序列预测，通过考虑未来外生变量和因果关系提升预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分利用未来外生变量和因果关系，导致预测性能不佳。

Method: 提出DAG框架，包含时间因果模块和通道因果模块，分别建模外生变量间的因果关系及其对内生变量的影响。

Result: 通过双因果网络，框架能够更有效地利用外生变量提升预测准确性。

Conclusion: DAG框架通过建模因果关系，显著提升了时间序列预测的准确性。
...
TL;DR: 比较基于编码器和解码器变换器的机器人检测模型效果，编码器模型更准确，解码器模型更具适应性。


<details>
  <summary>Details</summary>
Motivation: 社交媒体中机器人误导问题严重，需有效检测方法。

Method: 开发流程评估编码器和解码器变换器分类器性能。

Result: 编码器分类器更准确，解码器分类器更具适应性和泛化潜力。

Conclusion: 研究有助于防止数字环境被操纵，保护在线讨论的完整性。
...
TL;DR: 提出了一种基于社交网络和移动性的分层联邦学习框架HFL-SNM，优化资源分配和客户端调度，以减少能耗并提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统联邦学习框架常忽略客户端移动性，且假设本地数据绝对私有。本文旨在同时考虑数据共享和移动模式，优化资源受限下的联邦学习。

Method: 引入有效数据覆盖率和冗余数据覆盖率概念，通过初步实验分析数据对模型性能的影响，并提出动态优化算法DO-SNM。

Result: 实验表明，DO-SNM在显著降低能耗的同时，模型性能优于传统基线算法。

Conclusion: HFL-SNM框架和DO-SNM算法有效解决了移动性和数据共享问题，为资源受限的联邦学习提供了高效解决方案。
...
TL;DR: 研究使用集成机器学习技术预测埃塞俄比亚孕妇营养不良状况，随机森林模型表现最佳。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法难以捕捉营养状况的多维复杂因素，需更有效方法。

Method: 利用埃塞俄比亚人口与健康调查数据，预处理后应用多种集成算法（如XGBoost、随机森林等）。

Result: 随机森林模型准确率达97.87%，能有效分类四种营养状况。

Conclusion: 集成学习能捕捉复杂数据隐藏模式，为早期营养风险检测提供支持。
...
TL;DR: 论文提出了一种嵌套循环归一化随机双层近似方法（N²SBA），用于解决具有重尾噪声的随机双层优化问题，并扩展到非凸-强凹极小极大优化问题，达到了与已知最佳结果一致的复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决在重尾噪声环境下，随机双层优化和非凸-强凹极小极大优化问题的复杂性挑战，特别是在训练大型语言模型和强化学习等应用中。

Method: 提出N²SBA方法，利用归一化技术和嵌套循环结构，结合随机一阶梯度评估，处理重尾噪声。

Result: 在双层优化中达到SFO复杂度为$\tilde{\mathcal{O}}(\kappa^{\frac{7p-3}{p-1}} \sigma^{\frac{p}{p-1}} \epsilon^{-\frac{4p-2}{p-1}})$，在极小极大优化中达到$\tilde{\mathcal{O}}(\kappa^{\frac{2p-1}{p-1}} \sigma^{\frac{p}{p-1}} \epsilon^{-\frac{3p-2}{p-1}})$。

Conclusion: N²SBA方法在重尾噪声环境下表现优异，复杂度与已知最佳结果一致，适用于多种机器学习任务。
...
<details>
  <summary>Details</summary>
Motivation: 无线技术需要更智能地感知物理世界，但专用硬件成本高且复杂。ISAC被动感知提供了一种经济高效的解决方案，但现有方法局限于单一技术，限制了精度。

Method: 提出FAWN，基于Transformer架构，融合Wi-Fi和5G信息，实现多技术集成感知。

Result: 原型测试显示，84%的情况下误差低于0.6米。

Conclusion: FAWN通过多技术融合显著提升了ISAC被动感知的精度和覆盖范围。
...
Method: 将Adaptive Gradient Descent Without Descent方法扩展到随机设置，理论支持其收敛性。

Result: 在多种假设下证明了收敛性，并在实验中表现优于调优基线。

Conclusion: 该方法在理论和实践中均表现出色，适用于随机梯度优化。
...
TL;DR: 论文提出了一种名为AGCN的新架构，通过将注意力机制直接嵌入图结构，解决了GNN和Transformer在无监督图学习中的互补弱点，显著提升了图聚类性能。


<details>
  <summary>Details</summary>
Motivation: GNN和Transformer在图聚类任务中分别存在局部过强调和全局过强调的问题，导致性能不佳。论文旨在探索注意力机制在图学习中的潜力，并提出一种更有效的解决方案。

Method: 提出了Attentive Graph Clustering Network (AGCN)，将注意力机制嵌入图结构，结合KV缓存机制和成对边际对比损失，优化计算效率和注意力空间的判别能力。

Result: AGCN在实验中表现优于现有方法，验证了其有效性。

Conclusion: AGCN通过结合注意力机制与图结构，解决了GNN和Transformer的局限性，为无监督图学习提供了新的思路。
...
TL;DR: 提出了一种名为DEER的自适应经验回放框架，用于非平稳环境中的强化学习，通过区分策略更新和环境变化来提高学习效率。
...
Method: 提出了环境动态差异（DoE）指标，并基于此设计了DEER框架，使用二元分类器检测环境变化并应用不同的优先级策略。

Result: 在四个非平稳基准测试中，DEER将离策略算法的性能提升了11.54%。
...
### [117] [Beyond Marginals: Learning Joint Spatio-Temporal Patterns for Multivariate Anomaly Detection](https://arxiv.org/abs/2509.15033)
*Padmaksha Roy,Almuatazbellah Boker,Lamine Mili*

Main category: cs.LG

TL;DR: 提出了一种改进多元时间序列异常检测的方法，通过建模时变非线性时空相关性。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设时间序列变量独立，忽略了真实世界中的复杂交互。

Method: 使用Transformer编码器捕捉时间模式，结合多变量似然和copula建模空间依赖，通过对比学习联合训练。

Result: 模型能够捕捉集体行为异常，即使单个时间序列无明显异常。

Conclusion: 该方法有效提升了多元异常检测性能，通过解耦建模边际分布、时间动态和变量间依赖。
...
TL;DR: 提出两阶段框架，结合无监督模式提取与可解释预测，用于金融市场的方向性预测。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法泛化能力差和深度学习模型透明度低的问题。

Method: (i) SIMPC提取不变模式；(ii) JISC-Net基于shapelet分类预测短期方向。

Result: 在比特币和标普500股票数据中表现优异，11/12指标领先基线。

Conclusion: 方法透明且高效，揭示了驱动预测的模式结构。
...
### [119] [Reinforcement Learning Agent for a 2D Shooter Game](https://arxiv.org/abs/2509.15042)
*Thomas Ackermann,Moritz Spang,Hamza A. A. Gardi*

Main category: cs.LG

TL;DR: 提出了一种结合离线模仿学习和在线强化学习的混合训练方法，用于2D射击游戏智能体，显著提升了训练稳定性和胜率。
...
Conclusion: 结合演示初始化和强化学习优化是复杂多智能体环境中开发游戏AI的稳健解决方案。
...
TL;DR: 提出了一种结合监督微调和自我改进的两阶段后训练方法，显著提升了机器人控制的样本效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在机器人底层控制中主要局限于行为克隆，受大语言模型微调成功的启发，探索更高效的训练方法。

Method: 两阶段方法：1) 监督微调（SFT）结合行为克隆和步骤预测目标；2) 自我改进阶段，利用步骤预测提取奖励函数和成功检测器，实现自主练习。

Result: 实验表明，该方法比单纯模仿学习更高效，成功率更高，并能自主习得超越训练数据的新技能。

Conclusion: 结合预训练基础模型与在线自我改进，具有变革机器人自主技能获取的潜力。
...
TL;DR: 论文评估了五种机器学习模型在信用卡欺诈检测中的表现，混合方法在召回率和精确度上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 信用卡欺诈因类别不平衡和欺诈者模仿合法行为而难以检测，需有效方法提升模型性能。

Method: 使用逻辑回归、随机森林、XGBoost、KNN和MLP模型，结合欠采样、SMOTE和混合方法。

Result: 混合方法在召回率和精确度上表现最佳，显著提升了MLP和KNN的性能。

Conclusion: 混合方法在真实数据集中表现优异，为信用卡欺诈检测提供了有效解决方案。
...
TL;DR: 提出了一种稀疏RNN的替代超参数，通过隐藏比例指标提升性能。


<details>
  <summary>Details</summary>
Motivation: 改进稀疏RNN的性能，并引入隐藏比例指标以平衡模型中的未知分布。

Method: 使用可变稀疏度的RNN架构和隐藏比例指标。

Result: 显著提升性能，并为元学习和模型优化提供新路径。

Conclusion: 该方法为基于数据集内在特性的模型优化提供了新方向。
...
TL;DR: 提出了一种名为ADC的新型通信高效分割学习框架，通过双重压缩策略减少Vision Transformers激活的通信开销。


<details>
  <summary>Details</summary>
Motivation: 减少分割学习训练过程中传输中间Vision Transformers激活所需的通信开销。

Method: 采用两种并行压缩策略：基于注意力分数合并相似样本的激活（类无关），并丢弃最无意义的标记。

Result: ADC显著降低了通信开销，同时保持高准确性，优于现有分割学习框架。

Conclusion: ADC通过双重压缩策略有效减少了通信成本，且无需额外调整或梯度近似。
...
TL;DR: 论文提出了一种平滑的概率重构方法，用于解决ℓ0正则化回归问题，避免了蒙特卡洛采样，并实现了快速收敛。该方法在多种设置和信噪比下优于压缩感知算法。同时，论文探讨了非线性压缩感知中的参数恢复问题，发现非线性情况下存在对称性和反弹效应。
...
Method: 提出平滑概率重构方法，避免蒙特卡洛采样；研究非线性压缩感知中的参数恢复，基于Fefferman和Markel的理论。

Result: 新方法收敛速度显著提升，优于IHT和Lasso等算法；非线性压缩感知中参数恢复存在对称性和反弹效应。

Conclusion: 线性与非线性压缩感知存在本质差异，新方法为ℓ0正则化提供了高效解决方案。
...
Result: 在Abilene和GÉANT数据集上，RMSE分别降低了92%和75%；路由场景中最大链路利用率偏差减少了18%和21%。

Conclusion: 聚类方法能显著提升流量矩阵预测的准确性和实用性，尤其适用于网络优化场景。
...
TL;DR: 提出了一种在反馈受限的非平稳多臂老虎机中的新模型和算法，无需先验知识即可实现近最优动态遗憾。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设每轮都有反馈，但现实中反馈常受限，因此需要适应这种约束的算法。

Method: 提出首个无需非平稳性先验知识的算法，在反馈受限条件下优化动态遗憾。

Result: 算法达到动态遗憾$\tilde{\mathcal{O}}({K^{1/3} V_T^{1/3} T }/{ B^{1/3}})$，接近最优。

Conclusion: 该算法在反馈受限的非平稳环境中表现优异，为实际应用提供了有效解决方案。
...
TL;DR: AI驱动的代理通过天空图像预测空气污染水平，并生成逼真的污染场景可视化，结合统计纹理分析和监督学习，利用VLM引导的图像生成提高透明度。


<details>
  <summary>Details</summary>
Motivation: 传统空气污染监测系统空间覆盖有限，难以满足公众需求，需更直观、可访问的解决方案。

Method: 结合统计纹理分析和监督学习分类污染，利用VLM生成语义一致的污染可视化。

Result: 验证了方法在污染水平估计和视觉合成上的有效性，支持实时推断和用户交互。

Conclusion: 系统设计注重用户体验，未来将优化为绿色CNN架构，实现边缘平台实时部署。
...
TL;DR: FedLEASE提出了一种自适应分配和选择LoRA专家的联邦学习框架，解决了LLMs在异构数据下的微调问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在领域特定应用中需要大量数据，但数据可能分布在多个组织中，且联邦学习在LLMs上存在计算限制。LoRA虽高效，但单一模块难以应对异构数据。

Method: FedLEASE通过客户端表示相似性聚类分配领域特定LoRA专家，并引入自适应top-M MoE机制，让客户端选择最优专家数量。

Result: 实验表明，FedLEASE在异构客户端设置下显著优于现有联邦微调方法，同时保持通信效率。

Conclusion: FedLEASE有效解决了联邦LoRA微调中的专家分配和选择问题，提升了LLMs在异构数据下的性能。
...
TL;DR: 研究探讨在多智能体交互中，即使个体未完美对齐，通过模型多样性和战略竞争，用户仍能获得类似完美对齐的效用。


<details>
  <summary>Details</summary>
Motivation: 解决AI系统与人类价值观对齐的挑战，探索在不完美对齐情况下如何通过多智能体交互实现效用最大化。

Method: 采用多领导者Stackelberg博弈模型，扩展贝叶斯说服理论，分析多轮对话中的均衡条件。

Result: 证明在凸包条件下，用户可实现贝叶斯最优行动；非战略用户也能获得近似最优效用；实验验证理论。

Conclusion: 模型多样性和战略竞争可弥补个体对齐不足，为用户提供接近完美对齐的效用。
...
<details>
  <summary>Details</summary>
Motivation: 传统梯度训练方法效率低、能耗高，难以满足大规模模型（如LLMs）的可持续发展需求。

Method: 将神经网络分为两层：底层通过FPGA单步方程求解优化特征提取，高层采用增量学习支持持续更新。

Result: 显著降低计算成本，同时保持高性能，适用于边缘部署和实时适应。

Conclusion: 该框架提升了可持续性，减少了冗余计算，适合能源受限环境。
...
TL;DR: Super-Linear是一种轻量级、可扩展的混合专家模型，用于通用时间序列预测，通过频率专用线性专家和轻量级谱门控机制实现高效准确的预测。


<details>
  <summary>Details</summary>
Motivation: 现有大型预训练模型（如Chronos和Time-MoE）在零样本预测中表现良好，但计算成本高，需要更高效的解决方案。

Method: 使用频率专用线性专家替代深度架构，通过多频率数据训练，并结合轻量级谱门控机制动态选择专家。

Result: Super-Linear在保持高性能的同时，提供了更高的效率、对不同采样率的鲁棒性以及更好的可解释性。

Conclusion: Super-Linear是一种高效、轻量级的时间序列预测模型，适用于多种应用场景。
...
TL;DR: 论文分析了当前胸部X光公共数据集的局限性，包括标签质量、数据集偏差和领域偏移问题，并提出了改进建议。


<details>
  <summary>Details</summary>
Motivation: 现有胸部X光数据集存在标签错误、领域偏移和人口偏差等问题，影响了模型的泛化能力和临床实用性。

Method: 通过跨数据集领域偏移评估、源分类模型训练和专家评审，系统分析了数据集的问题。

Result: 研究发现模型在外部数据集上性能显著下降，存在明显的年龄和性别偏差，且专家与数据集标签存在显著分歧。

Conclusion: 当前基准数据集存在临床弱点，需要临床验证的数据集和更公平的评估框架。
...
TL;DR: TDRM通过最小化训练中的时间差异，学习更平滑和可靠的奖励模型，提升强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型缺乏时间一致性，导致策略更新无效和训练不稳定。

Method: 引入TDRM方法，通过时间差异（TD）正则化学习平滑奖励模型。

Result: 实验显示TDRM在多种设置中提升性能，并与RLVR结合实现更高效的数据利用。

Conclusion: TDRM是一种有效的奖励模型补充方法，显著提升语言模型策略质量。
...
TL;DR: 提出了一种混合架构框架，结合数字神经网络与物理层，通过随机零阶优化和动态低秩代理模型实现端到端训练。


<details>
  <summary>Details</summary>
Motivation: 物理设备在深度学习中的集成面临表达性有限和非可微性的挑战，需要开发新的训练方法。

Method: 使用随机零阶优化更新物理层参数，结合动态低秩代理模型实现梯度传播。

Result: 在多种任务中（如计算机视觉、音频分类和语言建模）接近数字基线准确率。

Conclusion: 该框架为集成非可微物理组件到可扩展AI系统提供了实用途径。
...
TL;DR: 论文提出了一种在标定集标签有噪声时应用共形预测（CP）的方法，通过数学推导估计无噪声CP阈值，并在医学影像数据集上验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 在高风险场景（如医学影像）中，回归模型的预测需要可靠的置信区间。现有CP方法在标定集标签有噪声时表现不佳，需改进。

Method: 提出数学方法估计无噪声CP阈值，并转化为实用算法，解决回归问题的连续性挑战。

Result: 在含高斯噪声的医学影像数据集上，新方法显著优于现有方法，接近无噪声标签的性能。

Conclusion: 新方法有效解决了标定集标签噪声问题，提升了CP在高风险回归任务中的可靠性。
...
TL;DR: 提出了一种新的低方差去偏方法，用于从标签比例学习（LLP），显著提升了现有技术水平。


<details>
  <summary>Details</summary>
Motivation: 在线广告中的问题促使研究从标签比例学习的任务。

Method: 引入了一种灵活的低方差去偏方法，适用于多种损失函数和分类场景。

Result: 通过结合标准技术，显著改善了样本复杂性，并在多个基准数据集上验证了方法的有效性。

Conclusion: 该方法在理论和实践中均表现出显著优势，优于现有基线方法。
...
TL;DR: 研究了联邦学习中基于logit的聚合方法，比较了三种方法，在减少通信开销和提高非IID数据下的鲁棒性方面表现良好。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中共享模型权重或梯度的高成本问题，以及异构客户端信息聚合的挑战。

Method: 引入并比较了三种logit聚合方法：简单平均、不确定性加权平均和学习的元聚合器。

Result: 在MNIST和CIFAR-10上评估，减少了通信开销，提高了非IID数据下的鲁棒性，准确性与集中式训练相当。

Conclusion: 基于logit的联邦学习方法在成本和性能上具有优势，适用于大规模模型和非IID数据场景。
...
### [138] [Mind the Gap: Data Rewriting for Stable Off-Policy Supervised Fine-Tuning](https://arxiv.org/abs/2509.15157)
*Shiwan Zhao,Xuyang Zhao,Jiaming Zhou,Aobo Kong,Qicheng Li,Yong Qin*

Main category: cs.LG

TL;DR: 论文提出了一种主动缩小策略差距的数据重写框架，通过保留正确解作为同策略数据并重写错误解，减少重要性采样方差，稳定了离策略微调。


<details>
  <summary>Details</summary>
Motivation: 监督微调（SFT）中，专家演示与目标策略的分布不匹配会导致高方差和训练不稳定，现有方法通过被动约束更新来缓解问题。

Method: 提出数据重写框架，主动缩小策略差距：保留正确解，重写错误解，仅在需要时回退到专家演示。

Result: 在五个数学推理基准测试中，相比普通SFT和动态微调（DFT），取得了显著且一致的性能提升。

Conclusion: 主动调整训练分布与目标策略对齐，有效减少了重要性采样方差，提升了离策略微调的稳定性。
...
### [139] [MaRVIn: A Cross-Layer Mixed-Precision RISC-V Framework for DNN Inference, from ISA Extension to Hardware Acceleration](https://arxiv.org/abs/2509.15187)
*Giorgos Armeniakos,Alexis Maras,Sotirios Xydis,Dimitrios Soudris*

Main category: cs.LG

TL;DR: 提出MaRVIn框架，通过硬件-软件协同设计优化混合精度神经网络在RISC-V架构上的执行，显著提升能效和性能。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入式微处理器缺乏对混合精度神经网络的高效支持，导致计算效率低下。

Method: 结合硬件改进（如可配置混合精度ALU）、混合精度量化、ISA优化和循环精确仿真，以及软件层面的剪枝感知微调和贪婪设计空间探索。

Result: 在CIFAR10和ImageNet上，平均加速17.6倍且精度损失小于1%，能效达1.8 TOPs/W。
...
### [140] [Evolving Language Models without Labels: Majority Drives Selection, Novelty Promotes Variation](https://arxiv.org/abs/2509.15194)
*Yujun Zhou,Zhenwen Liang,Haolin Liu,Wenhao Yu,Kishan Panaganti,Linfeng Song,Dian Yu,Xiangliang Zhang,Haitao Mi,Dong Yu*

Main category: cs.LG

TL;DR: EVOL-RL是一种无标签强化学习方法，通过结合稳定性和多样性，防止模型生成结果的熵崩溃，提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有无标签方法（如置信度最小化或多数投票）会导致探索能力下降和多样性崩溃，EVOL-RL旨在解决这一问题。

Method: EVOL-RL结合多数投票答案作为稳定锚点，并引入新颖性奖励以鼓励语义空间中的多样性，同时使用GRPO实现。

Result: EVOL-RL显著提升了模型性能（如AIME25 pass@1从4.6%提升至16.4%），并防止了多样性崩溃。

Conclusion: EVOL-RL在无标签和RLVR设置中均表现出色，具有广泛适用性。
...
### [141] [Explaining deep learning for ECG using time-localized clusters](https://arxiv.org/abs/2509.15198)
*Ahcène Boubekki,Konstantinos Patlatzoglou,Joseph Barker,Fu Siong Ng,Antônio H. Ribeiro*

Main category: cs.LG

TL;DR: 提出了一种新的可解释性方法，用于分析心电图（ECG）的卷积神经网络，通过提取时间局部化聚类和量化不确定性，增强对模型预测的理解和信任。
...
Method: 从模型的内部表示中提取时间局部化聚类，分割ECG波形，并量化这些表示的不确定性。

Result: 方法能够可视化不同波形区域对模型预测的贡献，并评估其决策的确定性。

Conclusion: 该方法为ECG深度学习模型提供了结构化和可解释的视角，增强了AI诊断的信任度，并有助于发现临床相关的电生理模式。
...
### [142] [CausalPre: Scalable and Effective Data Pre-processing for Causal Fairness](https://arxiv.org/abs/2509.15199)
*Ying Zheng,Yangfan Jiang,Kian-Lee Tan*

Main category: cs.LG

TL;DR: CausalPre是一种可扩展且高效的数据预处理框架，通过因果推理保证公平性，无需强假设因果模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法在因果公平性中依赖强假设或牺牲关系覆盖范围，CausalPre旨在解决这一问题。

Method: 通过分布估计问题提取因果公平关系，采用低维边际分解和启发式算法确保可扩展性。

Result: 在基准数据集上验证了CausalPre的有效性和可扩展性。

Conclusion: CausalPre证明无需牺牲关系覆盖范围即可实现因果公平性，挑战了传统观念。
...
### [143] [FlowRL: Matching Reward Distributions for LLM Reasoning](https://arxiv.org/abs/2509.15207)
...
TL;DR: FlowRL通过平衡奖励分布而非最大化奖励，提升LLM强化学习中的多样性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有奖励最大化方法（如PPO和GRPO）倾向于过度优化主导奖励信号，忽视低频但有效的推理路径，导致多样性降低。

Method: 将标量奖励转化为归一化目标分布，通过最小化反向KL散度实现流平衡优化，促进多样探索和泛化推理轨迹。

Result: 在数学和代码推理任务中，FlowRL平均比GRPO提升10.0%，比PPO提升5.1%。

Conclusion: 奖励分布匹配是实现高效探索和多样推理的关键步骤。
...
### [144] [AEGIS: Automated Error Generation and Identification for Multi-Agent Systems](https://arxiv.org/abs/2509.14295)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体系统（MAS）的复杂性和自主性增加，需要理解其错误模式以确保可靠性和安全性，但缺乏大规模、多样化的标注数据集。

Method: AEGIS框架利用LLM驱动的自适应操纵器，通过注入可控错误生成数据集，并探索监督微调、强化学习和对比学习三种范式。

Result: 实验表明，基于AEGIS数据的模型在所有学习范式中均有显著提升，部分模型性能优于更大规模的专有系统。
...
### [145] [FlowDrive: Energy Flow Field for End-to-End Autonomous Driving](https://arxiv.org/abs/2509.14303)
...
TL;DR: FlowDrive提出了一种基于能量流场的BEV表示方法，用于自动驾驶的运动规划，通过显式建模风险和引导先验，提升安全性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶框架在BEV空间中缺乏对风险和语义先验的显式建模，导致规划不够安全和可解释。

Method: FlowDrive引入风险势能和车道吸引力场等物理可解释的流场，结合条件扩散规划器，解耦运动意图预测和轨迹去噪。

Result: 在NAVSIM v2基准测试中，FlowDrive以86.3的EPDMS分数达到SOTA性能，显著提升了安全性和规划质量。

Conclusion: FlowDrive通过显式建模流场和扩散规划器，实现了更安全、可解释且多样化的自动驾驶运动规划。
...
### [146] [Multi-Quadruped Cooperative Object Transport: Learning Decentralized Pinch-Lift-Move](https://arxiv.org/abs/2509.14342)
...
Method: 采用分层策略架构，分离基础移动和手臂控制，提出星座奖励公式统一位置和方向跟踪。

Result: 在2-10个机器人团队中实现稳健运输，适用于不同几何形状和质量物体，并完成仿真到现实的轻量物体转移。

Conclusion: 通过奖励设计和训练课程，机器人表现出刚性连接行为，无需显式机械约束，实现无通信协调。
...
### [147] [LeVR: A Modular VR Teleoperation Framework for Imitation Learning in Dexterous Manipulation](https://arxiv.org/abs/2509.14349)
*Zhengyang Kris Weng,Matthew L. Elwin,Han Liu*

Main category: cs.RO

TL;DR: LeVR是一个模块化软件框架，旨在解决机器人模仿学习中的两个关键问题：提供VR遥操作数据收集，并与LeRobot模仿学习框架集成。


<details>
  <summary>Details</summary>
Motivation: 解决现有系统中机器人手臂与灵巧手结合的VR遥操作数据收集不足，以及模仿学习数据收集流程繁琐的问题。

Method: 开发LeVR框架，集成VR遥操作和LeRobot模仿学习框架，并发布LeFranX开源实现。

Result: 成功收集100个专家演示数据集，并用于微调先进的视觉运动策略。

Conclusion: LeVR框架和LeFranX开源实现为机器人模仿学习研究提供了高效工具，推动了该领域的发展。
...
### [148] [DreamControl: Human-Inspired Whole-Body Humanoid Control for Scene Interaction via Guided Diffusion](https://arxiv.org/abs/2509.14353)
...
TL;DR: DreamControl结合扩散模型和强化学习，通过人类运动数据训练的扩散先验指导RL策略，实现更自然和高效的人形机器人技能学习。


<details>
  <summary>Details</summary>
Motivation: 解决传统强化学习在复杂任务中难以发现自然动作的问题，同时提升仿真到现实的迁移能力。

Method: 利用人类运动数据训练的扩散先验指导RL策略，在仿真中完成特定任务。

Result: DreamControl在Unitree G1机器人上验证了其有效性，能够完成涉及上下肢协调和物体交互的复杂任务。

Conclusion: 扩散先验显著提升了RL的性能和动作的自然性，为仿真到现实迁移提供了新思路。
...
### [149] [CRAFT: Coaching Reinforcement Learning Autonomously using Foundation Models for Multi-Robot Coordination Tasks](https://arxiv.org/abs/2509.14380)
*Seoyeon Choi,Kanghyun Ryu,Jonghoon Ock,Negar Mehr*

Main category: cs.RO

TL;DR: CRAFT利用基础模型作为“教练”，通过LLM分解任务并生成奖励函数，VLM优化奖励，成功实现多机器人复杂协调任务。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习在机器人应用中面临高维连续动作空间和复杂奖励设计的挑战，受人类分阶段学习启发，提出CRAFT框架。

Method: CRAFT利用LLM分解长时程任务为子任务，生成奖励函数，并通过VLM优化奖励，训练多机器人协调行为。

Result: 在四足机器人导航和双手操作任务中验证了CRAFT学习复杂协调行为的能力，并在真实硬件实验中验证了导航策略。

Conclusion: CRAFT通过基础模型的推理能力，有效解决了多机器人协调任务中的挑战，展示了实际应用的潜力。
...
TL;DR: RLBind是一个两阶段对抗不变跨模态对齐框架，用于增强统一嵌入的鲁棒性，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 机器人感知需要多模态编码器，但视觉分支易受对抗性和自然损坏影响，现有防御方法效果有限。

Method: RLBind分两阶段：1) 无监督微调视觉编码器；2) 利用跨模态对应关系最小化特征差异并强制类别分布对齐。

Result: 在多种数据上，RLBind在干净精度和对抗鲁棒性上均优于基准方法。

Conclusion: RLBind为机器人多传感器感知提供了更安全的解决方案，同时保持泛化能力。
...
### [151] [GestOS: Advanced Hand Gesture Interpretation via Large Language Models to control Any Type of Robot](https://arxiv.org/abs/2509.14412)
*Artem Lykov,Oleg Kobzarev,Dzmitry Tsetserukou*

Main category: cs.RO

TL;DR: GestOS是一个基于手势的操作系统，用于智能控制异构机器人团队，通过语义解析手势并动态分配任务。


<details>
  <summary>Details</summary>
Motivation: 现有系统通常将手势映射为固定命令或单机器人动作，缺乏对多机器人团队的智能协调能力。

Method: 结合轻量级视觉感知和大型语言模型（LLM），将手势转换为结构化文本描述，由LLM推断意图并生成机器人特定命令。

Result: 系统实现了上下文感知的自适应控制，无需用户明确指定目标或命令。

Conclusion: GestOS通过从手势识别到智能协调的进步，支持在动态环境中与机器人系统的可扩展、灵活且用户友好的协作。
...
### [152] [Perception-Integrated Safety Critical Control via Analytic Collision Cone Barrier Functions on 3D Gaussian Splatting](https://arxiv.org/abs/2509.14421)
*Dario Tscholl,Yashwanth Nakka,Brian Gunter*

Main category: cs.RO

TL;DR: 提出了一种基于感知的安全过滤器，将3D高斯分布转换为碰撞锥，生成控制屏障函数，实现高效、平滑的避障。


<details>
  <summary>Details</summary>
Motivation: 解决传统距离基控制屏障函数反应滞后的问题，提升避障的主动性和平滑性。

Method: 利用3D高斯分布的解析几何特性，构建连续、闭式的碰撞约束，嵌入二次规划中。

Result: 在170k高斯分布的场景中，规划时间减少3倍，轨迹抖动显著降低，安全性不变。

Conclusion: 方法无需高阶扩展，适用于实时导航，尤其适合空间机器人和卫星系统。
...
### [153] [Local-Canonicalization Equivariant Graph Neural Networks for Sample-Efficient and Generalizable Swarm Robot Control](https://arxiv.org/abs/2509.14431)
*Keqin Wang,Tao Zhong,David Chang,Christine Allen-Blanchette*

Main category: cs.RO

TL;DR: LEGO框架通过图神经网络解决多智能体强化学习中的泛化和稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 解决竞争性多智能体任务中的训练不稳定、非动力学对策失效和泛化能力差的问题。

Method: 提出LEGO框架，结合图神经网络、规范化和异构表示。

Result: 在合作和竞争任务中表现优异，泛化能力提升。

Conclusion: LEGO在现实实验中展现出对团队规模和智能体故障的鲁棒性。
...
### [154] [Online Learning of Deceptive Policies under Intermittent Observation](https://arxiv.org/abs/2509.14453)
...
TL;DR: 研究如何在监督控制环境中，通过理论心智（Theory of Mind）引导在线强化学习实现欺骗行为。


<details>
  <summary>Details</summary>
Motivation: 在监督控制中，监控是间歇性的，代理可以利用这一点在不被察觉的情况下追求私人目标。

Method: 利用理论心智建模监督者的期望，并通过KL正则化策略改进步骤实现欺骗行为。

Result: 在海洋和空中导航的硬件实验中，方法成功实现了高回报和符合监督者期望的欺骗行为。

Conclusion: 理论心智可以有效地引导在线强化学习实现欺骗行为，且无需手动设计策略。
...
### [155] [Learning Discrete Abstractions for Visual Rearrangement Tasks Using Vision-Guided Graph Coloring](https://arxiv.org/abs/2509.14460)
*Abhiroop Ajith,Constantinos Chamzas*

Main category: cs.RO

TL;DR: 提出一种从视觉数据中自动发现抽象结构的方法，用于机器人重排任务的高效规划。


<details>
  <summary>Details</summary>
Motivation: 人类通过高层次抽象推理和低层次技能执行高效解决问题，而机器人中的抽象通常需要人工设计，限制了可扩展性和适用性。

Method: 结合结构约束和注意力引导的视觉距离，从原始图像中诱导离散的图结构抽象。

Result: 在模拟重排任务中，该方法能一致地识别有意义的抽象，支持高效规划并优于现有方法。

Conclusion: 自动化抽象发现可提升机器人规划框架的扩展性和适用性。
...
### [156] [Object Recognition and Force Estimation with the GelSight Baby Fin Ray](https://arxiv.org/abs/2509.14510)
*Sandra Q. Liu,Yuxiang Ma,Edward H. Adelson*

Main category: cs.RO

TL;DR: 论文探讨了利用机器学习提升软体机器人手和触觉传感器的性能，特别是GelSight Baby Fin Ray在纹理识别和力/位置估计中的应用。


<details>
  <summary>Details</summary>
Motivation: 研究旨在进一步探索GelSight Baby Fin Ray的潜力，通过机器学习提取触觉图像中的有用信息，以增强软体机器人对环境的理解和交互能力。

Method: 采用多种神经网络结构（如ResNet50、GoogLeNet和不同层数的CNN）进行消融实验，以区分坚果壳纹理并估计力和位置。

Result: 实验表明，机器学习能够有效从高分辨率触觉图像中提取信息，支持软体机器人更好地与环境互动。

Conclusion: 机器学习是提升软体机器人触觉感知和交互能力的有效方法。
...
### [157] [Event-LAB: Towards Standardized Evaluation of Neuromorphic Localization Methods](https://arxiv.org/abs/2509.14516)
*Adam D. Hines,Alejandro Fontan,Michael Milford,Tobias Fischer*

Main category: cs.RO

TL;DR: Event-LAB是一个统一框架，用于简化事件定位研究的代码和数据格式多样性问题，支持多种方法和数据集。


<details>
  <summary>Details</summary>
Motivation: 解决事件定位研究中代码和数据集格式多样性带来的比较困难问题。

Method: 使用Pixi包和依赖管理器实现统一框架，支持多种定位方法和数据集。

Result: 展示了框架在VPR和SLAM中的应用，揭示了事件生成参数对性能的影响。
...
### [158] [Learning to Pick: A Visuomotor Policy for Clustered Strawberry Picking](https://arxiv.org/abs/2509.14530)
...
<details>
  <summary>Details</summary>
Motivation: 草莓自然生长时密集且易被遮挡，传统机器人采摘系统难以应对这种复杂环境。

Method: 系统采用4自由度SCARA机械臂和人类远程操作接口收集数据，并利用改进的ACT模型开发精细视觉运动采摘策略。

Result: 实验表明，改进方法在多种遮挡场景下显著优于直接应用ACT模型。
...
### [159] [Dual-Arm Hierarchical Planning for Laboratory Automation: Vibratory Sieve Shaker Operations](https://arxiv.org/abs/2509.14531)
*Haoran Xiao,Xue Wang,Huimin Lu,Zhiwen Zeng,Zirui Guo,Ziqi Ni,Yicong Ye,Wei Dai*

Main category: cs.RO

TL;DR: 提出了一种分层规划框架，结合先验引导路径规划和多步轨迹优化，解决了振动筛分机自动化操作中的挑战。


<details>
  <summary>Details</summary>
Motivation: 解决振动筛分机自动化操作中的三个关键任务：狭窄空间的双臂操作、重叠工作区的双手交接以及受限的粉末容器交付。

Method: 采用先验引导路径规划（使用高斯混合模型提高采样效率）和多步轨迹优化（路径缩短、简化、关节约束和B样条平滑）。

Result: 规划时间减少80.4%，路径点减少89.4%，并在物理实验中完成完整工作流程。

Conclusion: 框架有效解决了复杂实验室自动化中的操作挑战，具有实际应用价值。
...
### [160] [SimCoachCorpus: A naturalistic dataset with language and trajectories for embodied teaching](https://arxiv.org/abs/2509.14548)
...
TL;DR: SimCoachCorpus是一个独特的赛车模拟驾驶数据集，用于研究在指导和未指导下的运动技能习得中的交互现象。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏语言与动作结合的领域，尤其是在通过语言指导习得运动技能方面。

Method: 29名参与者进行赛车模拟驾驶，其中15人接受专业教练的一对一指导，14人无指导。数据集包含车辆状态、地图、语音指导等。

Result: 数据集包含20,000多条实时反馈、400多条总结反馈及40多小时驾驶数据，可用于运动学习、语言现象研究和模型训练。

Conclusion: SimCoachCorpus为运动学习动态、语言现象和教学模型训练提供了自然数据集，未来将公开。
...
### [161] [Hierarchical Planning and Scheduling for Reconfigurable Multi-Robot Disassembly Systems under Structural Constraints](https://arxiv.org/abs/2509.14564)
...
<details>
  <summary>Details</summary>
Motivation: 针对可重构机器人在拆卸约束结构时面临的大规模复杂搜索空间和局部最优问题，提出了一种灵活的集成方法。

Method: 采用分层优化方法，结合多目标遗传算法进行序列和任务规划，以及约束编程进行调度，并引入染色体初始化方法以减少局部最优风险。

Result: 仿真结果表明，该方法能有效解决可重构机器人拆卸中的复杂问题。

Conclusion: 提出的系统集成方法在非破坏性拆卸约束结构中表现出高效性和适应性。
...
### [162] [Toward Embodiment Equivariant Vision-Language-Action Policy](https://arxiv.org/abs/2509.14630)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作策略在跨具身预训练中对新机器人配置的泛化能力有限，主要原因是忽视了动作空间的设计。

Method: 提出了一种等变性理论，设计了等变性动作解码器和几何感知网络架构。

Result: 实验表明，该方法提升了预训练效果，并能高效微调至新机器人配置。

Conclusion: 通过等变性设计，显著提升了跨具身预训练的泛化能力和效率。
...
### [163] [BEV-ODOM2: Enhanced BEV-based Monocular Visual Odometry with PV-BEV Fusion and Dense Flow Supervision for Ground Robots](https://arxiv.org/abs/2509.14636)
*Yufei Wei,Wangtao Lu,Sha Lu,Chenxiao Hu,Fuzhang Han,Rong Xiong,Yue Wang*

Main category: cs.RO

TL;DR: BEV-ODOM2通过密集BEV光流监督和PV-BEV融合，解决了现有BEV方法稀疏监督和信息丢失的问题，显著提升了单目视觉里程计性能。


<details>
  <summary>Details</summary>
Motivation: 现有BEV方法在单目视觉里程计中存在稀疏监督信号和透视到BEV投影中的信息丢失问题。

Method: 引入密集BEV光流监督和PV-BEV融合，保留6-DoF运动线索并保持尺度一致性。

Result: 在多个数据集上表现优异，RTE提升40%，并公开了新的ZJH-VO数据集。

Conclusion: BEV-ODOM2框架显著提升了BEV方法的性能，为未来研究提供了新数据集。
...
### [164] [Efficient 3D Perception on Embedded Systems via Interpolation-Free Tri-Plane Lifting and Volume Fusion](https://arxiv.org/abs/2509.14641)
...
<details>
  <summary>Details</summary>
Motivation: 现有三平面方法计算量大，不适合嵌入式3D推理，需要更高效的解决方案。

Method: 通过直接投影3D体素到平面特征并通过广播和求和重建特征体积，将非线性转移到2D卷积，降低复杂度。

Result: 分类和补全任务保持或提升精度，分割和检测任务以轻微精度下降换取显著计算节省。

Conclusion: 方法在嵌入式设备上实现实时吞吐量，适用于机器人感知。
...
### [165] [RealMirror: A Comprehensive, Open-Source Vision-Language-Action Platform for Embodied AI](https://arxiv.org/abs/2509.14687)
...
<details>
  <summary>Details</summary>
Motivation: 人形机器人的VLA研究面临数据获取成本高、缺乏标准化基准和仿真与现实差距大的挑战。

Method: 提出RealMirror平台，整合高效低成本的数据收集、模型训练和推理系统，并引入专用VLA基准。通过生成模型和3D高斯泼溅技术实现零样本Sim2Real迁移。

Result: 成功展示了无需微调的零样本Sim2Real迁移能力，模型在仿真数据上训练后可直接应用于真实机器人。

Conclusion: RealMirror为加速人形机器人VLA模型的发展提供了强大框架。
...
### [166] [exUMI: Extensible Robot Teaching System with Action-aware Task-agnostic Tactile Representation](https://arxiv.org/abs/2509.14688)
*Yue Xu,Litao Wei,Pengyu An,Qingyu Zhang,Yong-Lu Li*

Main category: cs.RO

TL;DR: 提出了一种结合硬件和算法创新的触觉机器人学习系统exUMI和TPP，解决了数据稀缺和稀疏性问题，并在真实实验中表现优于传统触觉模仿学习。


<details>
  <summary>Details</summary>
Motivation: 触觉机器人学习面临数据稀缺、稀疏性以及缺乏力反馈的挑战，需要新的硬件和算法解决方案。

Method: 开发了exUMI设备，增强数据收集能力；提出TPP框架，通过动作感知的时序触觉预测进行表示学习。

Result: exUMI实现了100%数据可用性，TPP在真实实验中优于传统方法。

Conclusion: 通过硬件和算法的协同设计，填补了人类触觉直觉与机器人学习之间的鸿沟，推动了接触丰富操作研究的发展。
...
### [167] [Wohlhart's Three-Loop Mechanism: An Overconstrained and Shaky Linkage](https://arxiv.org/abs/2509.14698)
*Andreas Mueller*

Main category: cs.RO

TL;DR: 本文重新分析了一个三环空间连杆机构，发现其在参考配置下具有5个微分自由度，但实际自由度为3，表明其是过约束的。机构在局部是光滑流形，且微分自由度局部恒定，因此是摇动的。


<details>
  <summary>Details</summary>
Motivation: 重新审视2004年提出的三环空间连杆机构，以明确其自由度特性和配置空间的局部行为。

Method: 通过局部分析，计算运动切锥和配置空间的局部近似，研究连杆机构的自由度。

Result: 机构在参考配置下微分自由度为5，实际自由度为3，配置空间局部光滑且微分自由度恒定，表明机构是摇动的。

Conclusion: 该连杆机构是过约束且摇动的，参考配置不是奇异点。
...
### [168] [Rethinking Reference Trajectories in Agile Drone Racing: A Unified Reference-Free Model-Based Controller via MPPI](https://arxiv.org/abs/2509.14726)
*Fangguo Zhao,Xin Guan,Shuo Li*

Main category: cs.RO

TL;DR: 提出了一种无参考的无人机竞速方法，通过将RL奖励塑造的目标直接整合到MPPI中，实现了时间最优竞速。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预计算的参考轨迹，限制了性能；RL研究发现模型控制器优化的是替代目标而非直接竞速目标。

Method: 将门进度目标整合到MPPI中，利用其采样特性优化非连续目标；建立统一框架比较三种目标函数。

Result: 无参考方法在竞速性能上与传统方法相当或更优。

Conclusion: 无参考方法在无人机竞速中具有竞争力，优于依赖参考的方法。
...
### [169] [Investigating the Effect of LED Signals and Emotional Displays in Human-Robot Shared Workspaces](https://arxiv.org/abs/2509.14748)
*Maria Ibrahim,Alap Kshirsagar,Dorothea Koert,Jan Peters*

Main category: cs.RO

TL;DR: 研究探讨了非语言沟通（如LED信号和情感显示）对人机协作的影响，发现情感显示虽增加互动感，但对任务表现影响有限。


<details>
  <summary>Details</summary>
Motivation: 在共享工作空间中，有效沟通对安全和效率至关重要，因此研究非语言沟通方式对人机交互的影响。

Method: 通过为机器人装备LED灯带和动画面部显示，设计三种条件（仅LED、LED加反应性情感显示、LED加预判性情感显示），并进行18人参与的实验。

Result: 情感显示增加了机器人的互动感，但对碰撞预期、沟通清晰度和任务效率无显著提升。

Conclusion: 情感线索虽能增强用户参与感，但在共享工作空间中对任务表现的改善作用有限。
...
### [170] [Designing Latent Safety Filters using Pre-Trained Vision Models](https://arxiv.org/abs/2509.14758)
*Ihab Tabbara,Yuxuan Yang,Ahmad Hamzeh,Maxwell Astafyev,Hussein Sibai*

Main category: cs.RO

TL;DR: 论文探讨了预训练视觉模型（PVRs）在视觉安全过滤器设计中的有效性，分析了不同训练方式的权衡，并评估了其在资源受限设备上的实用性。


<details>
  <summary>Details</summary>
Motivation: 确保视觉控制系统的安全性是部署于关键场景的主要挑战，而安全过滤器在视觉控制中的应用尚未充分探索。

Method: 使用PVRs作为分类器、HJ可达性安全过滤器和潜在世界模型的骨干，比较了从头训练、微调和冻结PVRs的效果。

Result: 评估了PVRs在不同任务中的表现，比较了学习世界模型和Q函数在切换安全策略中的优劣。

Conclusion: PVRs在视觉安全过滤器中具有潜力，但需考虑资源限制和实际部署的挑战。
...
### [171] [COMPASS: Confined-space Manipulation Planning with Active Sensing Strategy](https://arxiv.org/abs/2509.14787)
*Qixuan Li,Chen Le,Dongyue Huang,Jincheng Yu,Xinlei Chen*

Main category: cs.RO

TL;DR: COMPASS是一个多阶段探索与操作框架，通过智能采样规划提升受限环境中的操作成功率。


<details>
  <summary>Details</summary>
Motivation: 受限和杂乱环境中的操作因部分可观测性和复杂配置空间而具有挑战性，需要智能探索策略。

Method: 提出COMPASS框架，包括局部碰撞地图构建、多目标效用函数优化视角选择、约束操作优化策略。

Result: 仿真中操作成功率提升24.25%，真实实验验证了在受限环境中的主动感知与操作能力。

Conclusion: COMPASS通过多阶段策略有效解决了受限环境中的操作问题，具有实际应用潜力。
...
### [172] [Scalable Multi-Objective Robot Reinforcement Learning through Gradient Conflict Resolution](https://arxiv.org/abs/2509.14816)
*Humphrey Munn,Brendan Tidd,Peter Böhm,Marcus Gallagher,David Howard*

Main category: cs.RO

TL;DR: GCR-PPO改进PPO算法，通过多目标梯度分解和优先级解决冲突，提升机器人任务性能。


<details>
  <summary>Details</summary>
Motivation: 传统RL控制器将多目标聚合为单一奖励，导致调优困难和局部最优，限制了可扩展性。

Method: 提出GCR-PPO，使用多头评论家分解目标梯度，按优先级解决冲突。

Result: 在IsaacLab基准测试中，GCR-PPO平均提升9.5%性能，高冲突任务表现更优。

Conclusion: GCR-PPO有效解决多目标冲突，提升机器人任务的可扩展性和性能。
...
### [173] [CollabVLA: Self-Reflective Vision-Language-Action Model Dreaming Together with Human](https://arxiv.org/abs/2509.14889)
*Nan Sun,Yongchang Li,Chenxu Wang,Huiying Li,Huaping Liu*

Main category: cs.RO

TL;DR: CollabVLA是一个自反思的视觉-语言-动作框架，通过混合专家设计整合VLM反思推理和扩散动作生成，解决了现有VLA的局限性，提升了成功率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决现有视觉-语言-动作（VLA）框架的局限性，如领域过拟合、不可解释的推理和高延迟问题。

Method: 采用两阶段训练方法（动作基础和反思调优），结合VLM反思推理和扩散动作生成。

Result: 相比生成代理，时间减少约2倍，Dream计数减少约4倍，成功率和可解释性提高。

Conclusion: CollabVLA将VLA从黑盒控制器转变为能够与人类协作的辅助代理。
...
### [174] [PERAL: Perception-Aware Motion Control for Passive LiDAR Excitation in Spherical Robots](https://arxiv.org/abs/2509.14915)
*Shenghai Yuan,Jason Wai Hao Yee,Weixiang Guo,Zhongyuan Liu,Thien-Minh Nguyen,Lihua Xie*

Main category: cs.RO

TL;DR: PERAL是一种感知感知运动控制框架，通过被动激发LiDAR扫描，提升垂直扫描多样性，同时保持导航精度。


<details>
  <summary>Details</summary>
Motivation: 解决水平安装LiDAR在特征稀缺环境中性能下降的问题，避免传统方法的硬件成本和复杂性。

Method: 通过建模内部差速驱动与传感器姿态的耦合，叠加非周期性振荡到运动指令中。

Result: 实验显示96%的地图完整性、27%的轨迹跟踪误差减少，以及低功耗和低成本。

Conclusion: PERAL在无需专用硬件的情况下，显著提升了LiDAR-IMU系统的性能。
...
### [175] [Robot Control Stack: A Lean Ecosystem for Robot Learning at Scale](https://arxiv.org/abs/2509.14932)
...
TL;DR: 介绍了Robot Control Stack (RCS)，一个支持大规模通用策略机器人学习的轻量级生态系统，解决了传统机器人软件框架的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 传统机器人软件框架在大规模数据训练和模型为中心的机器学习流程中成为瓶颈，仿真环境对真实实验的支持有限。

Method: 设计了RCS，一个模块化、易扩展的分层架构，统一了仿真和物理机器人的接口，支持sim-to-real转移。

Result: RCS在VLA和RL策略开发周期中表现出良好的可用性和性能，并通过实验验证了仿真数据对真实策略性能的提升。

Conclusion: RCS为机器人学习研究提供了一个高效、灵活的工具，支持大规模训练和真实实验，推动了Vision-Language-Action模型的发展。
...
### [176] [CAD-Driven Co-Design for Flight-Ready Jet-Powered Humanoids](https://arxiv.org/abs/2509.14935)
...
<details>
  <summary>Details</summary>
Motivation: 优化喷气动力人形机器人的设计，使其能够高效执行动态约束轨迹。

Method: 使用实验设计生成5,000种几何变体，通过K-means聚类减少计算成本，采用NSGA-II算法进行多目标优化。

Result: 输出一组飞行就绪的人形机器人配置和验证的控制参数。

Conclusion: 框架为选择和实现可行的空中人形设计提供了结构化方法。
...
### [177] [A Novel Task-Driven Diffusion-Based Policy with Affordance Learning for Generalizable Manipulation of Articulated Objects](https://arxiv.org/abs/2509.14939)
*Hao Zhang,Zhen Kan,Weiwei Shang,Yongduan Song*

Main category: cs.RO

TL;DR: DART是一个结合扩散策略、功能学习和线性时序逻辑的新框架，用于提升灵巧操作的效率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决灵巧操作中多关节物体操作和跨类别泛化的挑战。

Method: 利用线性时序逻辑理解任务语义，功能学习确定最佳交互点，扩散策略泛化交互。

Result: DART在操作能力、泛化性能、迁移推理和鲁棒性上优于现有方法。

Conclusion: DART通过结合多种技术有效提升了灵巧操作的性能。
...
### [178] [Multi-CAP: A Multi-Robot Connectivity-Aware Hierarchical Coverage Path Planning Algorithm for Unknown Environments](https://arxiv.org/abs/2509.14941)
*Zongyuan Shen,Burhanuddin Shirose,Prasanna Sriganesh,Bhaskar Vundurthy,Howie Choset,Matthew Travers*

Main category: cs.RO

TL;DR: Multi-CAP是一种分层覆盖路径规划算法，通过连接感知方法协调多机器人，显著减少覆盖时间和路径重叠。


<details>
  <summary>Details</summary>
Motivation: 解决多机器人在未知大环境中协调覆盖的挑战，最小化路径长度和冲突。

Method: 构建动态邻接图，将环境划分为连接子区域，通过VRP分配子区域并生成非冲突路径。

Result: 仿真和硬件实验显示，Multi-CAP在覆盖时间、路径长度和重叠率上优于现有方法。

Conclusion: 连接感知图和全局路径规划器是性能提升的关键。
...
### [179] [Human Interaction for Collaborative Semantic SLAM using Extended Reality](https://arxiv.org/abs/2509.14949)
*Laura Ribeiro,Muhammad Shaheer,Miguel Fernandez-Cortizas,Ali Tourani,Holger Voos,Jose Luis Sanchez-Lopez*

Main category: cs.RO

TL;DR: HICS-SLAM是一种人机协作的语义SLAM框架，通过共享扩展现实环境实现实时协作，提升复杂环境中的地图精度和语义完整性。


<details>
  <summary>Details</summary>
Motivation: 现有语义SLAM系统在遮挡、数据不完整或几何模糊的真实场景中表现不佳，无法充分利用人类的高层次空间和语义知识。

Method: 提出基于图的语义融合方法，将人类干预与机器人感知结合，实现可扩展的协作。

Result: 在真实建筑工地数据集上，房间检测精度、地图精度和语义完整性均优于自动化基线。
...
### [180] [Exploratory Movement Strategies for Texture Discrimination with a Neuromorphic Tactile Sensor](https://arxiv.org/abs/2509.14954)
...
<details>
  <summary>Details</summary>
Motivation: 受人类探索策略启发，旨在提升机器人在复杂环境中的纹理分类能力。

Method: 使用NeuroTac传感器，测试六种探索动作（如滑动、旋转等），并评估其在固定和变化环境下的性能。

Result: 在复杂条件下，滑动+旋转动作达到87.33%的最高准确率，功耗仅为8.04 mW。

Conclusion: 滑动+旋转是神经形态触觉感知在纹理分类任务中的最优策略，有望增强机器人环境交互能力。
...
### [181] [Affordance-Based Disambiguation of Surgical Instructions for Collaborative Robot-Assisted Surgery](https://arxiv.org/abs/2509.14967)
*Ana Davila,Jacinto Colan,Yasuhisa Hasegawa*

Main category: cs.RO

TL;DR: 提出了一种用于手术机器人助手的框架，通过视觉上下文解析和消除外科医生口头指令的歧义，提高人机协作的安全性。


<details>
  <summary>Details</summary>
Motivation: 手术中口头指令的模糊性影响人机协作效果，需要一种方法消除歧义并确保患者安全。

Method: 采用基于两层次可供性推理的系统，结合多模态视觉-语言模型和工具能力知识库，并使用双重集保形预测方法提供统计置信度。

Result: 在胆囊切除术视频数据集上测试，实现了60%的歧义消除率，提升了手术室中人机交互的安全性。

Conclusion: 该框架有效解决了手术中口头指令的歧义问题，为安全的人机协作提供了可行方案。
...
### [182] [PA-MPPI: Perception-Aware Model Predictive Path Integral Control for Quadrotor Navigation in Unknown Environments](https://arxiv.org/abs/2509.14978)
*Yifan Zhai,Rudolf Reiter,Davide Scaramuzza*

Main category: cs.RO

TL;DR: PA-MPPI通过感知感知优化轨迹，提升四旋翼无人机在未知环境中的导航能力，比基线方法性能提升100%。


<details>
  <summary>Details</summary>
Motivation: 解决四旋翼无人机在未知环境中导航时因障碍物遮挡而无法探索的问题。

Method: 提出感知感知的MPPI（PA-MPPI），通过感知成本函数优化轨迹，探索未知区域。

Result: 硬件实验显示PA-MPPI在50Hz运行时性能提升100%，并能作为导航基础模型的安全策略。

Conclusion: PA-MPPI有效解决了未知环境中的导航问题，提升了四旋翼无人机的探索能力和安全性。
...
### [183] [M4Diffuser: Multi-View Diffusion Policy with Manipulability-Aware Control for Robust Mobile Manipulation](https://arxiv.org/abs/2509.14980)
...
<details>
  <summary>Details</summary>
Motivation: 解决单视图方法视野有限和经典控制器在奇异点附近效率低的问题。

Method: 采用多视图扩散策略生成任务相关目标，并通过ReM-QP控制器高效执行。

Result: 实验显示M4Diffuser成功率提高7-56%，碰撞减少3-31%。

Conclusion: M4Diffuser为移动机器人在非结构化环境中的可靠操作提供了新途径。
...
### [184] [The Role of Touch: Towards Optimal Tactile Sensing Distribution in Anthropomorphic Hands for Dexterous In-Hand Manipulation](https://arxiv.org/abs/2509.14984)
*João Damião Almeida,Egidio Falotico,Cecilia Laschi,José Santos-Victor*

Main category: cs.RO

TL;DR: 研究了触觉反馈在机器人手部不同区域（手指和手掌）对物体重新定向任务的影响，分析了不同触觉配置对深度强化学习控制策略的影响。


<details>
  <summary>Details</summary>
Motivation: 探索触觉传感器在机器人手部不同区域的配置对精确控制的影响，尤其是非指尖区域的触觉反馈常被忽视。

Method: 通过分析不同触觉配置对深度强化学习控制策略的影响，研究物体特性与传感器最佳位置的关系。

Result: 确定了哪些触觉配置能提高操作的效率和准确性。

Conclusion: 为仿人末端执行器的设计和应用提供了有价值的见解，以增强其操作能力。
...
### [185] [ExT: Towards Scalable Autonomous Excavation via Large-Scale Multi-Task Pretraining and Fine-Tuning](https://arxiv.org/abs/2509.14992)
*Yifan Zhai,Lorenzo Terenzi,Patrick Frey,Diego Garcia Soto,Pascal Egli,Marco Hutter*

Main category: cs.RO

TL;DR: ExT框架通过大规模演示收集和预训练，结合微调技术，实现了自主挖掘机的多任务策略，展示了从仿真到现实的迁移能力。


<details>
  <summary>Details</summary>
Motivation: 解决自主挖掘机在不同工作场景和硬件配置下的适应性问题，减少手动调优的需求。

Method: 使用大规模专家演示进行预训练，并通过监督或强化学习微调策略以适应新任务。

Result: ExT策略在仿真和现实中均表现出色，能快速适应新任务和配置。

Conclusion: ExT为可扩展和通用的自主挖掘机技术奠定了基础。
...
### [186] [Semantic-LiDAR-Inertial-Wheel Odometry Fusion for Robust Localization in Large-Scale Dynamic Environments](https://arxiv.org/abs/2509.14999)
...
Method: 结合语义体素地图和改进的扫描匹配算法，使用紧耦合多传感器融合的iESKF，并引入3D自适应缩放策略。
...
### [187] [Online Multi-Robot Coordination and Cooperation with Task Precedence Relationships](https://arxiv.org/abs/2509.15052)
*Walker Gosrich,Saurav Agarwal,Kashish Garg,Siddharth Mayya,Matthew Malencia,Mark Yim,Vijay Kumar*

Main category: cs.RO

TL;DR: 提出了一种新的多机器人任务分配方法，结合任务间的复杂依赖关系、高效的任务内协调和机器人联盟合作，通过任务图和奖励函数建模，并设计了高效的网络流算法和在线迭代重分配算法。


<details>
  <summary>Details</summary>
Motivation: 解决多机器人任务分配中复杂任务关系、协调和合作的问题，提高任务执行效率和鲁棒性。

Method: 使用任务图建模任务关系，奖励函数描述联盟规模和前置任务效果，提出网络流算法和在线迭代重分配算法。

Result: 在随机任务和奖励函数的测试中，算法表现优于混合整数求解器和贪婪启发式方法，并在模拟器中验证了其高效性和鲁棒性。

Conclusion: 该方法能有效建模复杂任务关系，高效生成高保真任务计划，并利用任务关系提升性能。
...
### [188] [Ask-to-Clarify: Resolving Instruction Ambiguity through Multi-turn Dialogue](https://arxiv.org/abs/2509.15061)
...
<details>
  <summary>Details</summary>
Motivation: 现有VLA代理多为单向执行指令，无法处理模糊指令，需开发能与人类协作的智能体。

Method: 结合VLM和扩散模型，通过两阶段知识隔离策略训练，先解决模糊指令再生成动作。

Result: 在8个真实任务中表现优于现有VLA方法。

Conclusion: Ask-to-Clarify框架为协作型智能体提供了可行路径。
...
### [189] [Energy-Constrained Navigation for Planetary Rovers under Hybrid RTG-Solar Power](https://arxiv.org/abs/2509.15062)
*Tianxin Hu,Weixiang Guo,Ruimeng Liu,Xinhang Xu,Rui Qian,Jinyu Chen,Shenghai Yuan,Lihua Xie*

Main category: cs.RO

TL;DR: 提出了一种能量约束的轨迹规划框架，用于行星探测车在混合RTG-太阳能输入下的能量感知自主规划。


<details>
  <summary>Details</summary>
Motivation: 未来的行星探测车需要在混合RTG和太阳能输入下长时间运行，但现有规划方法通常忽略能量可行性，仅关注地形或几何平滑性。

Method: 通过将基于物理的平移、旋转和阻力功率模型与子系统负载结合，将累积能量预算和瞬时功率约束集成到SE(2)多项式轨迹优化中。

Result: 在月球类似地形上的仿真结果表明，该规划器生成的轨迹峰值功率仅超出规定限制0.55%，而现有方法超出17%以上。

Conclusion: 该方法为长期行星任务提供了一种原理性和实用性的能量感知自主规划解决方案。
...
### [190] [AnoF-Diff: One-Step Diffusion-Based Anomaly Detection for Forceful Tool Use](https://arxiv.org/abs/2509.15153)
*Yating Lin,Zixuan Huang,Fan Yang,Dmitry Berenson*

Main category: cs.RO

TL;DR: AnoF-Diff是一种基于扩散模型的方法，用于从时间序列数据中提取力-扭矩特征并检测异常，在嘈杂数据中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理真实世界中的嘈杂、非平稳和多变的传感器数据时效果不佳。

Method: 基于扩散模型提取力-扭矩特征，并用于异常检测。

Result: 在四个强力工具使用任务中，AnoF-Diff在F1-score和AUROC上优于其他方法，且对噪声数据更鲁棒。

Conclusion: AnoF-Diff在在线异常检测中表现出色，适用于强力工具使用场景。
...
### [191] [Parallel Simulation of Contact and Actuation for Soft Growing Robots](https://arxiv.org/abs/2509.15180)
*Yitian Gao,Lucas Chen,Priyanka Bhovad,Sicheng Wang,Zachary Kingston,Laura H. Blumenschein*

Main category: cs.RO

TL;DR: 本文提出了一种统一建模框架，用于软生长机器人的生长、弯曲、驱动和障碍物接触的集成建模，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 软生长机器人（藤蔓机器人）在非结构化和动态环境中表现出卓越的交互能力，但需要主动转向以应对复杂环境。

Method: 扩展了梁矩模型以包含驱动对生长过程中运动学的影响，并开发了快速并行仿真框架。

Result: 通过设计优化任务，找到了在杂乱环境中导航的机器人设计，减少了所需驱动器的数量，并展示了设计的鲁棒性。

Conclusion: 优化的设计在障碍物丰富的环境中成功部署，验证了框架的实用性和鲁棒性。
